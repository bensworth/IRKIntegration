...
linear PDEs with no algebraic constraint, we do want to highlight that the theory
and framework developed in this paper is fundamental to a forthcoming paper on
...
using a high-order discontinuous Galerkin finite element method. In particular, we demonstrate
...
\end{equation}
where $\bm\beta(\bm x)$ is a prescribed velocity field, and $\varepsilon$ is the diffusion coefficient.
Periodic boundary conditions are enforced on $\partial\Omega$.
Equation \eqref{eq:adv-diff} is discretized with an upwind discontinuous Galerkin method \cite{Cockburn2001}.
The diffusion terms are treated with the symmetric interior penalty method \cite{Arnold1982,Arnold2002}.
The resulting finite element problem is to find $u_h \in V_h$ such that, for all $v_h \in V_h$,
\[
	\begin{multlined}
	...
	\end{multlined}
\]
where $V_h$ is the DG finite element space consisting of piecewise polynomials of degree $p$ defined on elements of the computational mesh $\mathcal{T}$ of the domain $\Omega$.
No continuity is enforced between mesh elements.
Here, $\nabla_h$ is the broken gradient, $\Gamma$ denotes the skeleton of the mesh, and $\{ \cdot \}$ and $\llbracket \cdot \rrbracket$ denote the average and jump of a function across a mesh interface.
$\widehat{u_h}$ is used to denote the upwind numerical flux.
The resulting linear system is nonsymmetric and possesses an elementwise block structure, which makes it ammenable to use with block preconditioners.
This discretization has been implemented in the MFEM finite element framework \cite{Anderson2020}, and uses algebraic multigrid preconditioning with approximate ideal restriction (AIR) \cite{Manteuffel:2019,Manteuffel:2018}.

The DG method is particularly advantageous for advection-dominated problems.
In this example, we choose diffusion coefficient $\varepsilon = 10^{-6}$ and velocity field $\bm\beta(x,y) = (\cos(4\pi y), \sin(2 \pi x))^T$.
The velocity field, initial condition, and numerical solution are shown in \Cref{fig:ad_advdiff}.
...
It is well-known that matrix assembly becomes prohibitively expensive for high-order finite element.
...
On the other hand, matrix-free operator evaluation on tensor-product meshes can be performed in $\mathcal{O}(p^{d+1})$ operations \cite{Orszag1980}, motivativing the development of solvers and preconditioners that can be constructed and applied without access to the assembled system matrix \cite{Kronbichler2019}.
...
The matrix-free action of the corresponding operator is computed in $\mathcal{O}(p^{d+1})$ operations using the \textit{partial assebmly} features of the MFEM finite element library \cite{Anderson2020}.
...
We also use this test case to study the effect of subiterations on the convergence of the iteration solver.
...
For DIRK methods, the use of subiterations did not reduce the total number of preconditioner applications.
...
\begin{figure}
	...
	\caption{
		Comparison of number of iterations with and without inner subiterations, using the high-order finite element diffusion test problem.
