...
\Cref{ass:eig} and \ref{ass:fov} guarantee the methods proposed here yield
...
\OK{Change section title to ``The optimal choice of $\gamma$''? Or something to this effect.}
...
\mathcal{P}_\gamma & =
\Big[(\eta I - \widehat{\mathcal{L}})^2 + \beta^2 I\Big] (\gamma I - \widehat{\mathcal{L}})^{-2} \\ \nonumber
& =\Big[((\eta-\gamma)I + (\gamma I - \widehat{\mathcal{L}}))^2 + \beta^2 I\Big]  (\gamma I - \widehat{\mathcal{L}})^{-2} \\
...
\end{align}
\OK{I changed the above and the equation for the operator in the previous section to use right preconditioning rather than left for consistency since this is what I use in the proof. But the operator is identical for left v.s right preconditioning is the same. Why is this?}
%
...
\label{th:cond}
Suppose \Cref{ass:eig,ass:fov} hold, that is, $\eta > 0$ and $W(\cL) \leq 0$. Let $\mathcal{P}_\gamma$ denote
the preconditioned operator \eqref{eq:P_gamma}, where $[(\eta + \mathrm{i}\beta)I -
\cL][(\eta - \mathrm{i}\beta)I - \cL]$ is 
preconditioned with $(\gamma I - \cL)^{-2}$.
%
Let $\kappa({\cal P}_{\gamma})$ denote the two-norm condition number of ${\cal P}_{\gamma}$, and define $\gamma_*$ by 
\begin{align} \label{eq:gammastar}
\gamma_* \coloneqq \sqrt{\eta^2+\beta^2}.
\end{align}
Then
\begin{align} \label{eq:P_gammastar}
\kappa(\mathcal{P}_{\gamma_*}) \leq \sqrt{1+\frac{\beta^2}{\eta^2}}.
\end{align}

Moreover, bound \eqref{eq:P_gammastar} is tight since there exist matrices $\cL$ that satisfy it exactly. Finally, $\gamma = \gamma_*$ is optimal in the sense that it minimizes a tight upper bound on $\kappa({\cal P}_{\gamma})$ over all $\gamma \in (0, \infty)$ without further assumptions on $\cL$.
\end{theorem}
\begin{proof}
First we will prove bound \eqref{eq:P_gammastar}, then we will demonstrate it is tight, and finally we will demonstrate the optimality of $\gamma=\gamma_*$.
...
\end{align}
The key step in establishing \eqref{eq:P_gammastar} is bounding $\Vert {\cal P}_{\gamma} \Vert^2$ and $\Vert {\cal P}_{\gamma}^{-1} \Vert^2$ from above by bounding $\frac{\Vert {\cal P}_{\gamma} \bm{v} \Vert^2 }{\Vert \bm{v} \Vert^2}$ from above and below, respectively.
%
Considering the form of the preconditioned operator ${\cal P}_{\gamma}$ in \eqref{eq:P_gamma}, we make the substitution $\bm{v} = (\gamma I - \cL)^{2} \bm{w}$. Now, assuming $\bm{v}$ (and hence $\bm{w}$ also) is real, the squared norm of ${\cal P}_{\gamma} \bm{v}$ can be expressed as
\begin{align}
\label{eq:norm_Pv}
...
&=
\nonumber
\bVert (\eta^2 + \beta^2) \bm{w} + \cLs \bm{w} \bVert^2  - 4 \eta \langle \cL (\cL \bm{w}), \cL \bm{w} \rangle + 4 \eta^2 \bVert \cL \bm{w} \bVert^2 - 4 \eta (\eta^2 +\beta^2) \langle \cL \bm{w}, \bm{w} \rangle.
\end{align}
Similarly, the squared norm of $\bm{v}$ can be written as (e.g., by letting $\eta \mapsto \gamma$ and $\beta \mapsto 0$ in the right side of the equations above)
\begin{align}
\label{eq:norm_v}
\Vert \bm{v} \Vert^2
&=
\bVert ( \gamma I - \cL)^2 \bm{w} \bVert^2, \\
&=
\nonumber
\bVert \gamma^2 \bm{w} + \cLs \bm{w} \bVert^2  - 4 \gamma \langle \cL ( \cL \bm{w}), \cL \bm{w} \rangle + 4 \gamma^2 \bVert \cL \bm{w} \bVert^2 - 4 \gamma^3 \langle \cL \bm{w}, \bm{w} \rangle.
\end{align}
Thus, the key term from \eqref{eq:kappa_def} can be written in the form
\begin{align}
...
\begin{aligned}
\label{eq:f_c_def}
f_0 &\coloneqq \bVert \gamma^2 \bm{w} + \cLs \bm{w} \bVert^2 > 0,
\quad
&&c_0\coloneqq \frac{\bVert (\eta^2 + \beta^2) \bm{w} + \cLs \bm{w} \bVert^2}{\bVert \gamma^2 \bm{w} + \cLs \bm{w} \bVert^2} > 0,
\\
f_1 &\coloneqq -4 \gamma \langle \cL( \cL \bm{w}), \cL \bm{w} \rangle \geq 0,
\quad 
&&c_1\coloneqq \frac{\eta}{\gamma} > 0,\\
f_2 &\coloneqq 4 \gamma^2 \bVert \cL \bm{w} \bVert^2 \geq 0, 
\quad
&&c_2\coloneqq \frac{\eta^2}{\gamma^2} > 0, \\
f_3 &\coloneqq -4 \gamma^3 \langle \cL \bm{w}, \bm{w} \rangle \geq 0, 
\quad
&&c_3\coloneqq \frac{\eta (\eta^2 + \beta^2)}{\gamma^3} > 0,
\end{aligned}
\end{equation}
assuming $\gamma > 0$. Functions $f_1$ and $f_3$ are non-negative by assumption of $W( \cL ) \leq 0$.
%
Note that $f_0$ is positive because it would be zero only if $(\mathrm{i} \gamma, \bm{w})$ was an eigenpair of $\cL$, but this would violate the assumption that $ \cL \bm{w}$ is real (which is a consequence of $\bm{w}$ and $\cL$ being real). It is for analogous reasons that $c_0 > 0$. 

Since all of the addends in the numerator and denominator of \eqref{eq:P_gamma_frac} are non-negative, and at least one addend in each is positive, it can be bounded as
\begin{align*}
...
\end{align*}
Applying these bounds to the squared norms in \eqref{eq:kappa_def} yields
...
\end{align}
%
Bounding $c_0$ for general $\gamma$, and hence $c_{\min}$ and $c_{\max}$, is difficult because the sign of $\langle \cLs \bm{w}, \bm{w} \rangle$ (which appears in expanding $\bVert \gamma^2 \bm{w} + \cLs \bm{w} \bVert^2$) is not known for general $\cL$, noting that the sign of $W(\cL)$ does not determine that of $W(\cLs)$. However, observe from \eqref{eq:f_c_def} that $c_0(\bm{w}) = 1$ for the judicious choice of $\gamma = \gamma_* \coloneqq \sqrt{\eta^2 + \beta^2}$. Moreover, in the final part of this proof we will demonstrate that $\gamma = \gamma_*$ is optimal, and, as such, moving forward we analyze \eqref{eq:P_gamma_frac} only for the special case of $\gamma = \gamma_*$.

Letting $\gamma = \gamma_*$, from \eqref{eq:f_c_def} one has $1 = c_0 > c_1 = c_3 = \sqrt{c_2}$. Thus, for $\gamma = \gamma_*$, bounds \eqref{eq:P_gamma_bounds} are
\begin{align}
...
\end{align}
Applying these bounds to the condition number \eqref{eq:kappa_def} yields the claimed upper bound of \eqref{eq:P_gammastar}.

We now move toward showing that bound \eqref{eq:P_gammastar} is tight and that $\gamma = \gamma_*$ is optimal, which will do by placing lower bounds on $\kappa({\cal P}_{\gamma})$ for a particular class of matrices $\cL$. 
Consider again the key fraction of $\frac{\Vert {\cal P}_{\gamma} \bm{v} \Vert^2 }{\Vert \bm{v} \Vert^2}$ from the condition number \eqref{eq:kappa_def}. Again, let $~\bm{v}=(\gamma I - \cL)^2 \bm{w}$, but now suppose that $(\textrm{i} \xi, \bm{w})$ is an eigenpair of $\cL$, with $\xi$ a real number and $\bm{w}$ a complex vector. We use this eigenpair with \eqref{eq:norm_Pv} and \eqref{eq:norm_v} to define the following function
\begin{align} \label{eq:H_def}
{\cal H}_{\gamma}(\xi) 
\coloneqq 
\left. \frac{\Vert {\cal P}_{\gamma} \bm{v} \Vert^2 }{\Vert \bm{v} \Vert^2} \right|_{\cL \bm{w} = \mathrm{i} \xi \bm{w}}
=
\frac{|(\eta- \mathrm{i} \xi)^2 + \beta^2|^2}{|(\gamma - \mathrm{i} \xi)^2|^2}
=
\frac{(\eta^2 + \beta^2 - \xi^2)^2 + 4 \eta^2 \xi^2}{(\gamma^2-\xi^2)^2 + 4 \gamma^2 \xi^2}.
\end{align}
By virtue of restricting that $\bm{w}$ be an eigenvector, from \eqref{eq:kappa_def} we have
\begin{align}
\label{eq:H_bounds}
\frac{1}{\Vert {\cal P}_{\gamma}^{-1} \Vert^2}
=
\underset{\bm{v} \neq 0}{\min} \frac{\Vert {\cal P}_{\gamma} \bm{v} \Vert^2}{\Vert \bm{v} \Vert^2}
\leq {\cal H}_{\gamma}(\xi) \leq 
\underset{\bm{v} \neq 0}{\max} \frac{\Vert {\cal P}_{\gamma} \bm{v} \Vert^2}{\Vert \bm{v} \Vert^2} = \Vert {\cal P}_{\gamma} \Vert^2.
\end{align}
That is, any value of $\tfrac{1}{{\cal H}_{\gamma}(\xi)}$ serves as a lower bound on $\Vert {\cal P}_{\gamma}^{-1} \Vert^2$, while any value of ${\cal H}_{\gamma}(\xi)$ serves as a lower bound on $\Vert {\cal P}_{\gamma} \Vert^2$. Therefore, the ratio of any two values of ${\cal H}_{\gamma}(\xi)$ provide a lower bound on $\kappa^2({\cal P}_{\gamma})$. We remark that previously assuming $\bm{v}$ was real while now considering it complex is consistent because the min/max of $\frac{\Vert {\cal P}_{\gamma} \bm{v} \Vert^2}{\Vert \bm{v} \Vert^2}$ for real-valued ${\cal P}_{\gamma}$ is equivalent for $\bm{v}$ real or complex.

We now show that bound \eqref{eq:P_gammastar} on $\kappa^2({\cal P}_{\gamma_*})$ is tight. Considering \eqref{eq:H_def} at the judiciously chosen eigenvalues of $\mathrm{i} \xi = 0, \pm \mathrm{i} \gamma_*$, we have
\begin{align}
\label{eq:H1}
{\cal H}_{\gamma}(0) 
&= 
...
\label{eq:H2}
{\cal H}_{\gamma}(\pm \gamma_*) 
&= 
\frac{4 \eta^2 ( \eta^2 + \beta^2)}{(\gamma^2 + \eta^2 + \beta^2)^2}, 
\end{align}
First observe from \eqref{eq:H_bounds} and \eqref{eq:H1} that $\Vert {\cal P}_{\gamma_*} \Vert^2 \geq {\cal H}_{\gamma_*}(0) = 1$, and thus the upper bound on $\Vert {\cal P}_{\gamma} \Vert^2$ from \eqref{eq:P_gamma*_bounds} achieves equality for a matrix $\cL$ having an eigenvalue of $\mathrm{i} \xi = 0$. Secondly, observe from \eqref{eq:H_bounds} and \eqref{eq:H2} that $\Vert {\cal P}_{\gamma_*}^{-1} \Vert^2 \geq \tfrac{1}{{\cal H}_{\gamma_*}(\pm \gamma_*)}= 1 + \tfrac{\beta^2}{\eta^2}$, and thus the upper bound on $\Vert {\cal P}_{\gamma}^{-1} \Vert^2$ from \eqref{eq:P_gamma*_bounds} achieves equality for a matrix $\cL$ having eigenvalues $\mathrm{i} \xi = \pm \mathrm{i} \gamma_*$. Therefore, bound \eqref{eq:P_gammastar} on $\kappa({\cal P}_{\gamma_*})$ achieves equality for any matrix $\cL$ having eigenvalues $\mathrm{i}\xi =0, \pm \mathrm{i}\gamma_*$.

Finally, having shown that \eqref{eq:P_gammastar} is tight, we now show that  $\gamma = \gamma_*$ is optimal. Once again, consider the matrix $\cL$ from above with eigenvalues $\mathrm{i}\xi =0, \pm \mathrm{i}\gamma_*$, such that $\kappa^2({\cal P}_{\gamma_*})
=
1 + \frac{\beta^2}{\eta^2}$. Now, observe from this, \eqref{eq:H_bounds}, \eqref{eq:H1}, and \eqref{eq:H2} that for such a matrix
\begin{align}
\label{eq:kappa_lwr_bnd1}
\kappa^2({\cal P}_{\gamma_*})
<
\frac{(\eta^2 + \beta^2) (\gamma^2 + \eta^2 + \beta^2)^2}{4 \eta^2 \gamma^4}
=
\frac{{\cal H}_{\gamma}(0)}{{\cal H}_{\gamma}(\pm \sqrt{\eta^2 + \beta^2})}
\leq
\kappa^2({\cal P}_{\gamma}),
\quad 0 < \gamma < \gamma_*.
\end{align}
Now let us further impose that $\cL$ has eigenvalues of $\mathrm{i} \xi \to \pm \mathrm{i} \infty$, which, when substituted into \eqref{eq:H_def}, yields
\begin{align}
\label{eq:H3}
\lim_{\xi \to \pm \infty} {\cal H}_{\gamma}(\xi) = 1.
\end{align}
Now upon combining \eqref{eq:H_bounds}, \eqref{eq:H2}, and \eqref{eq:H3}, observe that
\begin{align}
\label{eq:kappa_lwr_bnd2}
\kappa^2({\cal P}_{\gamma_*})
< 
\frac{(\gamma^2 + \eta^2 + \beta^2)^2}{4 \eta^2 (\eta^2 + \beta^2)}
=
\frac{{\cal H}_{\gamma}(\pm \infty)}{{\cal H}_{\gamma}(\pm \sqrt{\eta^2 + \beta^2})}
\leq
\kappa^2({\cal P}_{\gamma}), 
\quad \gamma_* < \gamma < \infty.
\end{align}
Thus, from \eqref{eq:kappa_lwr_bnd1} and \eqref{eq:kappa_lwr_bnd2}, a matrix $\cL$ having eigenvalues $\mathrm{i}\xi =0, \pm \mathrm{i}\gamma_*, \pm \mathrm{i} \infty$ satisfies $\kappa({\cal P}_{\gamma}) > \kappa({\cal P}_{\gamma_*}) = \sqrt{1 + \beta^2/\eta^2}$ for $\gamma \in (0, \infty) \setminus \gamma_*$. It therefore holds for general $\cL$ that a tight upper bound on $\kappa({\cal P}_{\gamma})$ for $\gamma \in (0, \infty) \setminus \gamma_*$ must be larger than the tight upper bound of $\kappa({\cal P}_{\gamma_*}) \leq \sqrt{1 + \beta^2/\eta^2}$. Hence $\gamma=\gamma_*$ is the minimizer over $\gamma \in (0, \infty)$ of a tight upper bound on $\kappa({\cal P}_{\gamma})$ for general matrices $\cL$.
\end{proof}
%


In \cite{exh}, similar analysis \OK{It's probably no longer appropriate to call their analysis ``similar''?} is done under the assumption
...
\textnormal{cond}(\mathcal{P}_{\gamma = \eta}) < 1 + \frac{\beta^2}{\eta^2}.
\end{align}
%
...
in \eqref{eq:cond_gen} to $\gamma_*$ exactly yields \eqref{eq:P_gammastar}.
Altogether, this suggests the condition number bounds in \Cref{th:cond}
are moderately tight.
\OK{Not sure how much of the above paragraph you want to keep since some of it no longer applies.}
...
\OK{Not sure how much of the above paragraph you want to keep; e.g., I assume you don't want to discuss $W(\cLs)$ any more since we don't have those bounds any more?}

\Cref{tab:cond} provides condition number bounds from \Cref{th:cond} and
\eqref{eq:P_gammastar} for Gauss, Radau IIA, and Lobatto IIIC Runge-Kutta methods.
...
Lobatto IIIC  & 2.24 & 1.00 & 3.37 & 4.37 & 1.19 & 1.00 & 5.27 & 1.52 \\\hline
  \end{tabular}
  \caption{\OK{Update the numbers in table using new bound. I assume you already have code for this somewhere, Ben? For consistency, probably best you do it.} Bounds on $\kappa(\mathcal{P}_{\gamma_*})$ from \Cref{th:cond} and
  \eqref{eq:P_gammastar} for Gauss, Radau IIA, and Lobatto IIIC integration,
...
Speedup & 1.0 & 1.0 & 3.0 & 1.2 & 4.3 & 1.0 & 1.4 & 5.8 \\\hline\hline
  ...
  $(\gamma_* M - \delta t \mathcal{L})^{-2}$, with $\gamma_*$ defined in \eqref{eq:gammastar}. 
