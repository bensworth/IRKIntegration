...
Thus, consider a similar preconditioner but with preconditioning constant
$\gamma > 0$, where the preconditioned operator takes the form
%
\begin{align}\label{eq:P_gamma}
\mathcal{P}_\gamma & =
	(\gamma I - \widehat{\mathcal{L}})^{-2}\Big[(\eta I - \widehat{\mathcal{L}})^2 +
	\beta^2 I\Big].
% 	\\
% & = (\gamma I - \widehat{\mathcal{L}})^{-2}\Big[((\eta-\gamma)I +
...
% 	\left(I - \tfrac{1}{\gamma}\widehat{\mathcal{L}}\right)^{-2}. \nonumber
\end{align}
%
Such an approach was proven effective for symmetric definite spatial matrices in
\cite{exh}, with optimal constant $\gamma_* = \sqrt{\eta^2+\beta^2}$. The following
theorem derives tight bounds on the condition number of $P_{\gamma}$ as well as 
optimality of $\gamma\in(0,\infty)$ in a much more general context.

%
\begin{theorem}[Optimal preconditioning]
\label{th:cond} Let $\cL$ be real-valued and suppose \Cref{ass:eig,ass:fov} hold,
...
%
Let $\kappa({\cal P}_{\gamma})$ denote the two-norm condition number of ${\cal P}_{\gamma}$,
...
%
Consider the form of the preconditioned operator ${\cal P}_{\gamma}$ in
...
\bVert (\eta^2 + \beta^2) \bm{w} + \cLs \bm{w} \bVert^2  -
	4 \eta \langle \cL (\cL \bm{w}), \cL \bm{w} \rangle + \\&\hspace{15ex}
...
\end{aligned}
\end{equation}
assuming $\gamma > 0$. Note that functions $f_1$ and $f_3$ are non-negative by
assumption of $W( \cL ) \leq 0$, while for all $\mathbf{w}$, it must hold that
either $f_2 > 0$ or $c_0f_0 > 0$ (or both).
...
choice of $\gamma = \gamma_* \coloneqq \sqrt{\eta^2 + \beta^2}$ yields $c_0(\bm{w}) = 1$.
...
Letting $\gamma = \gamma_*$, from \eqref{eq:f_c_def} one has $1 = c_0 > c_1 = c_3
= \sqrt{c_2}$. Thus, for $\gamma = \gamma_*$, the bounds in \eqref{eq:P_gamma_bounds}
are given by
\begin{align}
...
problems. Indeed, one example in \Cref{sec:numerics:dg} demonstrates an almost
...
Lobatto IIIC & 1.41 & 1.00 & 1.79 & 2.12 & 1.06 & 1.00 & 2.42 & 1.17 \\\hline
  \end{tabular}
  \caption{Bounds on $\kappa(\mathcal{P}_{\gamma_*})$ from \Cref{th:cond} and
