...
fully implicit Runge-Kutta methods and discontinuous Galerkin discretizations in time
...
\Cref{sec:solve} then introduces a new method to solve for the IRK update
...
operator proven to have condition number $<9$, independent of the number of
...
up to 10th-order accuracy in time, and, in addition, for one example requires
...
% ------------------------------------------------------------------------------------- %
\subsubsection{Preconditioning with $\eta$ vs. $\gamma_*$}\label{sec:numerics:dg:const}

First, we demonstrate the effectiveness of using $\gamma_*$ (\Cref{sec:solve:gamma})
instead of $\eta$ in the preconditioner. 
...
\begin{table}[!ht]
  \centering
  \begin{tabular}{| c | cc | cc | ccc |}  % chktex 44
  \hline
\multicolumn{8}{|c|}{Gauss}\\\hline
2 & \multicolumn{2}{c}{3} & \multicolumn{2}{|c}{4} & \multicolumn{3}{|c|}{5} \\\hline
1 & . & . & . & . & . & . & . \\\hline\hline
\multicolumn{8}{|c|}{RadauIIA}\\\hline
2 & \multicolumn{2}{c}{3} & \multicolumn{2}{|c}{4} & \multicolumn{3}{|c|}{5} \\\hline
1 \\\hline\hline
\multicolumn{8}{|c|}{LobattoIIIC}\\\hline
2 & \multicolumn{2}{c}{3} & \multicolumn{2}{|c}{4} & \multicolumn{3}{|c|}{5} \\\hline
  \hline
  \end{tabular}
  \caption{.}\label{tab:gamma}
\end{table}
}
...
\todo{Add similar plot for $\kappa 10^{-6}$.}
