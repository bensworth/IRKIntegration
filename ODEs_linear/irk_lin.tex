...
the preconditioned operator, where $((\eta + \mathrm{i}\beta)I - \widehat{\mathcal{L}})((\eta - \mathrm{i}\beta)I - \widehat{\mathcal{L}})$ is
...
$\tfrac{1}{\gamma^2}\widehat{\mathcal{L}}^2\mathbf{w}$ were kept together in norm 
to avoid the difficulties that arise in analyzing
$\langle\widehat{\mathcal{L}}^2\mathbf{w},\mathbf{w}\rangle$). In addition, at least 
one term in the numerator and denominator is positive, because if
$\mathcal{L}\mathbf{w}=\mathbf{0}$ for $\mathbf{w}\neq\mathbf{0}$, then
$(1+c_I)\mathbf{w}+\tfrac{1}{\gamma^2}\widehat{\mathcal{L}}^2\mathbf{w} = 
(1+c_I)\mathbf{w}\neq \mathbf{0}$. 

Here we have a fraction of the form $(C_1x_1 + C_2x_2 + C_3x_3 + C_4x_4)/
(K_1\hat{x}_1 + K_2x_2 + K_3x_3 + K_4x_4)$ for $C_i,x_i\geq 0 $, $K_i > 0$, and at
least one triplet, e.g., $\{x_1, C_1, K_1\}$, strictly  positive. Then,
%
\begin{align}\label{eq:khat}
\frac{x_1 + C_2x_2 + C_3x_3 + C_4x_4}{\hat{x}_1 + K_2x_2 + K_3x_3 + K_4x_4} \geq \min_{C_ix_i\neq 0}
	\left\{\frac{x_1}{\hat{x}_1},\frac{C_2}{K_2},\frac{C_3}{K_3},\frac{C_4}{K_4}\right\}
:= \widehat{K}
\end{align}
%
...
corresponding to $x_2, x_3$, and $x_4$, respectively, we can directly bound the ratio of constants
in the numerator and denominator:
%
\begin{align*}
\frac{C_2}{K_2} & = \frac{(\tfrac{2}{\gamma}-c_{\widehat{\mathcal{L}}})^2}{\frac{4}{\gamma^2}}
	 = \frac{\eta^2}{\gamma^2},
\hspace{5ex}
\frac{C_4}{K_4} = \frac{2\tfrac{2-\gamma c_{\widehat{\mathcal{L}}}}{\gamma^3}}{\frac{4}{\gamma^3}}
	= \frac{\eta}{\gamma}, 
\\
& \frac{C_3}{K_3}  = \frac{2(1+c_I)(\tfrac{2}{\gamma}-c_{\widehat{\mathcal{L}}})}{\frac{4}{\gamma}}
	= \frac{\eta(\eta^2+\beta^2)}{\gamma^3}, 
\end{align*}
%
For $\gamma \in[\eta,\sqrt{\eta^2+\beta^2}]$, the minimum of the
above ratios is $\tfrac{\eta^2}{\gamma^2}$.

The difficult ratio to bound in \eqref{eq:smin_ex} ends up being
$x_1 := \|(1+c_I)\mathbf{w}+\tfrac{1}{\gamma^2}\widehat{\mathcal{L}}^2\mathbf{w}\|^2$ and the
similar denominator term, $\hat{x}_1 :=
\|\mathbf{w}+\tfrac{1}{\gamma^2}\widehat{\mathcal{L}}^2\mathbf{w}\|^2$.
Here we consider two cases:\\
\\
%%%%%
\underline{$\boldsymbol{\gamma = \gamma_* = \sqrt{\eta^2+\beta^2}}$}: Under this specific choice, we
get the nice cancellation that $c_I = 0$, and have matching terms
$\|\mathbf{w}+\tfrac{1}{\gamma_*^2}\widehat{\mathcal{L}}^2\mathbf{w}\|^2$ in the numerator
and denominator of \eqref{eq:smin_ex}. Then, $x_1/\hat{x}_1 = 1$ and the minimum ratio of
numerator to denominator terms in \eqref{eq:khat} is $\widehat{K} := \tfrac{\eta^2}{\gamma_*^2}$.
From \eqref{eq:khat} and \eqref{eq:smin_ex} we can bound $\|P_{\gamma_*}^{-1}\|$ via
%
\begin{align}\label{eq:gammas_bound}
\|P_{\gamma_*}^{-1}\| & = \frac{1}{s_{\min}(P_{\gamma_*})}
	\leq \frac{1}{\sqrt{\widehat{K}}}
...
%%%%%
\noindent
\underline{\textbf{Nonnegative $\widehat{\mathcal{L}}^2$}}: Expanding $x_1/\hat{x}_1$
yields
%
{\small
\begin{align}\label{eq:bad}
\frac{x_1}{\hat{x}_1} = \frac{\|(1+c_I)\mathbf{w}+\tfrac{1}{\gamma^2}\widehat{\mathcal{L}}^2\mathbf{w}\|^2}
	{\|\mathbf{w}+\tfrac{1}{\gamma^2}\widehat{\mathcal{L}}^2\mathbf{w}\|^2}
& = \frac{(1+c_I)^2\|\mathbf{w}\|^2+\tfrac{1}{\gamma^4}\|\widehat{\mathcal{L}}^2\mathbf{w}\|^2
	+ \tfrac{2(1+c_I)}{\gamma^2}\langle\widehat{\mathcal{L}}^2\mathbf{w},\mathbf{w}\rangle}
	{\|\mathbf{w}\|^2+\tfrac{1}{\gamma^4}\|\widehat{\mathcal{L}}^2\mathbf{w}\|^2
	+ \tfrac{2}{\gamma^2}\langle\widehat{\mathcal{L}}^2\mathbf{w},\mathbf{w}\rangle}.
\end{align}
}
%
By assumption $W(\widehat{\mathcal{L}}^2) \geq 0$ and, analogous to \eqref{eq:khat},
we have a fraction $(A_1y_1+A_2y_2+A_3y_3)/(B_1y_1+B_2y_2+B_3y_3)$ for $A_i,B_i>0$
and $y_i\geq 0$, with at least one $y_i > 0$. Recall for $\gamma \in[\eta,\gamma_*]$,
$c_I \geq 0$. Thus $\tfrac{A_1}{B_1},\tfrac{A_2}{B_2},\tfrac{A_3}{B_3} \geq 1$,
and it follows that $x_1/\hat{x}_1 \geq 1$ \eqref{eq:bad}.
%
...
% $s_{\min}(\mathcal{P}_\gamma)$.
Similar to the proof for $\gamma = \gamma_*$, the minimum ratio of
numerator to denominator terms in \eqref{eq:khat} is
again $\widehat{K} := \tfrac{\eta^2}{\gamma^2}$. Altogether, this yields
%
\begin{align*}
\|P_{\gamma}^{-1}\| & = \frac{1}{s_{\min}(P_{\gamma})}
	\leq \frac{1}{\sqrt{\widehat{K}}}
	= \frac{\gamma}{\eta}.
\end{align*}
%
...
Speedup & 1.0 & 1.0 & 3.0 & 1.2 & 4.3 & 1.0 & 1.4 & 5.8 \\\hline\hline
  ...
  $(\gamma_* M - \delta t \mathcal{L})^{-2}$, with $\gamma_*$ defined in \eqref{eq:gam_opt}. 
