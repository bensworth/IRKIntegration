...
theorem derives tight bounds on the condition number of a slightly more general operator than ${\cal P}_{\gamma}$ (an operator of which ${\cal P}_{\gamma}$ is a special case) as well as 
optimality of $\gamma\in(0,\infty)$ in a much more general context. The corollary that follows it shows that the form of preconditioner considered in ${\cal P}_{\gamma}$  \eqref{eq:P_gamma} is optimal with $\gamma = \gamma_*$.
...
\begin{corollary}[Optimal preconditioning with respect to \eqref{eq:P_gamma}]
\label{cor:cond}
...
\end{align}
That is to say, the preconditioner $(\gamma I - \cL)^{-2}$ employed in \eqref{eq:P_gamma} with $\gamma = \gamma_*$ is optimal over the space of general preconditioners $(\delta I - \cL)^{-1}(\gamma I - \cL)^{-1}, \, \delta, \gamma \in (0,\infty)$ employed in \eqref{eq:P_gen}. Furthermore, the condition number of the preconditioned operator ${\cal P}_{\gamma}$ \eqref{eq:P_gamma} when $\gamma = \gamma_*$ is tightly bounded as
