...
A field of values analysis is derived to guarantee rapid Krylov convergence under
quite general assumptions on the spatial discretization that yield stable time integration.
A modified constant is introduced to the preconditioner, and the resulting preconditioned
operator proven to have condition number $\mathcal{O}(1)$ (and $<9$), independent of
the number of stages or polynomial order.
The new method is demonstrated to be effective
on various high-order finite-difference and finite-element discretizations of linear
...
focuses on spatial operators that are independent of time \OK{Is it clear to the reader that by `spatial operators' you mean ${\cal L}$ and not $f$?}; however, some of
...
parabolic PDEs where $\mathcal{L}$ is symmetric positive definite (SPD) \OK{Isn't $-{\cal L}$ SPD rather than ${\cal L}$?}, \eqref{eq:k0}
instead yields a large nonsymmetric system with significant block coupling. For
nonsymmetric systems $\mathcal{L}$ \OK{`systems' $\to$ `matrices'?} that already have variable coupling, fast iterative
...
high-order finite-difference and discontinuous Galerkin spatial discretizations of
advection-diffusion equations, and very high-order matrix-free continuous Galerkin
discretizations of diffusion equations \OK{Is 10th-order CG really `very high order,' while 8th-order FD only `high order'?}. The method is shown to be fast and scalable
...
schemes suffer from order reduction. The stage-order of a DIRK method is at most one
(EDIRK methods, with one explicit stage, have a maximum stage order of two) \cite{butcher00}.
For stiff nonlinear PDEs or DAEs, the observed
global order of accuracy
in practice can be limited to $\approx \min\{ p, q+1\}$ or $q$, respectively, for
integration order $p$ and stage-order $q$ \cite{hairer96,kennedy16} \WP{the previous sentence was not so clear}. Thus, even a
...
straightforward to satisfy in practice \OK{Should it be clarified that this assumption is also fulfilled for SDIRK methods? The reader may be left with the impression that it's only fulfilled for IRK methods.}.
...
block-diagonal mass matrix inverse \WP{why is the mass matrix block diagonal?}, and let
%
\begin{equation*}
...
Then, solving \eqref{eq:k0} can be effectively reduced to inverting the operator \OK{Do you mean \eqref{eq:keq} rather than \eqref{eq:k0}?}
%
\begin{align}\nonumber
...
\begin{align*}
\mathcal{M}_s^{-1} = \det(\mathcal{M}_s)^{-1}\textnormal{adj}(\mathcal{M}_s).
\end{align*}
%
...
\end{lemma}
%
\OK{I recall we had a discussion whether it was clearer to say the det of ${\cal M}_s$ was a block diagonal matrix, or to use the correct notation $\textnormal{diag}( \det({\cal M}_s))$; I said I thought the latter was less confusing and easier to understand, and you said you'd consider changing it. It's fine either way, I just wanted to remind you in case you forgot about it. See also my point on the next page about this.}
\begin{proof}
...
restrict our attention to the case $\widehat{\mathcal{L}}_i = \widehat{\mathcal{L}}_j$ for all $i,j$, \OK{Should we clarify why this is the case? I.e., that we don't really know how to compute the det for time-dependent operators?}
in which case $\det(\mathcal{M}_s)^{-1} = P_s(\widehat{\mathcal{L}})^{-1}$, where
$P_s(x)$ is the characteristic polynomial of $A_0^{-1}$ (see \Cref{lem:inv}). \OK{Relating to my earlier point on Lemma \ref{lem:inv}, this equation now makes moving from the first equation in the Lemma to the second inconsistent since there's a diag missing from around $P_s(\widehat{\mathcal{L}})^{-1}$.}
...
\end{split}
\end{align}
\OK{I think $\eta$ and $\beta$ need to be defined since they're central to the rest of the paper. So something like: `for $\lambda = \eta + i \beta$, where $\eta, \beta \in \mathbb{R}$.' should follow this equation?}
%
...
\textnormal{ }\newline
The conditioning of $\mathcal{P}_\eta$ depends on the ratio
...
Suppose Assumptions \ref{ass:eig} and \ref{ass:fov} hold, that is, $\eta > 0$ \OK{But this is not what Assumption 1 states: It says that the eigenvalues of $A_0$ have real positive part, not that those of $A_0^{-1}$ do. So maybe clarify that this assumption also implies that the eigenvalues $A_0^{-1}$ have such a property too?}
...
Let $\pi_k$ denote the set of consistent polynomials of degree $k$ \WP{Define consistent polynomials?}. Then the ideal GMRES bound
...
with preconditioned operator $\mathcal{P}_\eta$, error is bounded by (cf.\ \cite{Trefethen1997})
...
we would like to obtain convergence independent of $\beta$ and $\eta$, analogous
...
decomposition \OK{There is no Schur decomposition stuff in this paper; that's for the nonlinear/time-dependent one?}, and, for SPD operators, a modified constant $\eta \mapsto
\sqrt{\eta^2+\beta^2}$ is derived, \OK{Maybe change to `a modified preconditioning constant'? It kind of reads like you're changing the value of $\eta$.} where the preconditioned operator has
condition number $<2$ \OK{Is this really a 2, or should it be a 9?}, independent of $\eta$ and $\beta$. In this section,
...
of $\eta$ and $\beta$. \OK{Aren't these last few sentences just saying the same things twice?}
...
be any good reason to use $\gamma$ outside of this range (for details,
see the discussion following \Cref{th:cond}, and in \cite{exh} for SPD operators).
...
operators, or a flaw in the line of proof) \WP{Clarify this setence?}.
...
respect to \eqref{eq:cond1}. \OK{Just a thought: Have you tried out the optimal $\gamma$/at least looked at what the condition number bounds are (by numerically solving for it)? If so, is it much better than $\gamma_*$? It's probably worth adding an option into the code to do this; I can do it, just let me know if you think it's worthwhile.}
...
$(\eta I - \widehat{\mathcal{L}})^{-1}$. \OK{Don't we precondition with $(\eta I - \widehat{\mathcal{L}})^{-2}$, or two applications of $(\eta I - \widehat{\mathcal{L}})^{-1}$, rather than by $(\eta I - \widehat{\mathcal{L}})^{-1}$?} However, for $\beta > \eta$, conditioning
...
$(\gamma_* I - \widehat{\mathcal{L}})^{-1}$, \OK{As above, should it be $(\gamma_* I - \widehat{\mathcal{L}})^{-2}$?} regardless of $\eta$ and $\beta$.
...
and can be solved using preconditioned conjugate gradient \OK{Why now `conjugate gradient' rather than `CG'?} or MINRES, preconditioned
...
\OK{This section should be updated to use $\gamma$ rather than $\eta$ when discussing the preconditioner, including the equation below for $B^{-2}\mathcal{Q}_\eta$.}
...
inverse approximation}. Fortunately, for difficult problems without highly effective
...
\cite{Notay2000,saad1993flexible}) to ensure robust (outer) iterations, analogous
...
using multiple ``inner'' preconditioning iterations to approximate $\widehat{L}^{-1}$ \OK{What is $\widehat{L}^{-1}$?}
...
\begin{figure}[!htb]
  ...
  \end{subfigure}
  \\\vspace{2ex}
      \caption{\OK{Consider describing as `DG advection-diffusion problem' or something similar rather than just `advection-diffusion problem' to distinguish it from the FD advection-diffusion problem?} Advection-diffusion problem with velocity field shown in
...
solving two different linear systems and SDIRK4 only one, \OK{Radau9 solves 3 different systems, not 2?} very high-order
...
methods. \OK{Besides the abstract, this is the only time you use the term `backward Euler time step'; Will the reader know what you mean?} Fortunately, ensuring a preconditioner is sufficiently good can be
...
AIR iterations applied for each application of the preconditioner. \OK{Describe what $\varepsilon$ means here/what its relevance is (e.g., left is advection-dominated, right is mixed)?}}
\label{fig:dg_o2}
\end{figure}
...
numerical PDEs. Theory is developed to guarantee rapid Krylov convergence and
$\mathcal{O}(1)$ conditioning under general assumptions on the spatial
discretization that yield stable time integration.
