...
The resoluts are presented in Table \ref{tab:ev-errors}.
...
In Table \ref{tab:ev-solvers}, we present the number of nonlinear iterations required to converge with a representative time step of $\Delta t = 2 \times 10^{-2}$, together with the total number of Krylov iterations in one step.
We note that the second- and fourth-order Gauss methods require fewer total Krylov iterations when compared with the equal-order SDIRK methods.
Similarly, the third-order Radau IIA method requires one fewer Krylov iteration when compared with the third-order SDIRK method.
...
\begin{table}
	...
	\begin{tabular}{r|cccc|ccccc}
		...
		\midrule
		Total Krylov its. & 20 & 26 & 45 & 59 & 15 & 36 & 103 & 162 & 169\\
		\bottomrule
	\end{tabular}
	...
	\begin{tabular}{r|cccc|cccc}
		...
		\midrule
		Total Krylov its. & 44 & 121 & 168 & 205 & 66 & 225 & 210 & 292\\
		\bottomrule
	\end{tabular}
\end{table}
...
As a more challening test case, we consider the Reynolds number 40{,}000 flow over a NACA0012 airfoil.
...
% ------------------------------------------------------------------------------------- %
% ------------------------------------------------------------------------------------- %
\subsection{Incompressible Euler in vorticity-streamfunction form}\label{sec:numerics:khi}
...
Note that this formulation can be easily extended to the 2D incomressible Navier--Stokes equations with the addition of a viscosity term.
...
\end{equation}
where $M_{\rm dg}$ represents the DG mass matrix, $M_{\rm mix}$ is the mixed DG-$H^1$ mass matrix, $K(\psi)$ is the discretized advection operator (depending the velocity $\bm u$ as a function of $\psi$), and $A$ is the $H^1$-conforming diffusion operator.
...
Therefore, the $4\times4$ block system \eqref{eq:dae_block} that arises from IRK integration can be rearragned to be written as
\begin{align}
\begin{bmatrix}
