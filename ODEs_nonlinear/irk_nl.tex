...
-\frac{\beta^2}{\phi} I & \eta I - \widehat{\mathcal{L}}_1\end{bmatrix},
\end{align}
%
where it is assumed that $W(\widehat{\mathcal{L}}) \geq 0$ in \Cref{ass:fov}. In some
...
this sytem using Krylov methods with block lower-triangular preconditioners of the form
%
...
\end{equation}
%
where $\widehat{S}$ is some approximation to the Schur complement, and the Schur
complement of \eqref{eq:block0} given by
%
...
% ---------------------------------------------------------------------------------------------- %
% ---------------------------------------------------------------------------------------------- %
\subsection{A modified $\gamma$: the SPD case}\label{sec:theory:spd}
...
\end{align}
%
where $\lambda\in\sigma(-\mathcal{L})$. If we choose $\gamma\geq\eta$,
...
derives a closed form for $c(\gamma)$ and correpsoinding optimal
(minimizing) value of $\gamma$.
...
\gamma_*  :=  \underset{\eta \leq \gamma < \infty}{\textnormal{argmin}} \; c(\gamma)
	= \eta + \frac{\beta^2}{\eta},
\end{align}
where
...
Thus moving forward we consider $\eta > \gamma$.
...
\tcb{(BS: next two equations not obvious, is first one relevant?)}
...
\end{align*}
%
Furthermore, any such roots must satisfy the quadratic equation in $\hat{\gamma}$
%
\begin{align*}
...
\not\in(\eta,\eta+\beta^2/\eta]$ and, thus, ${\cal G}$ has no sign changes
...
\end{align*}
%
Combining with \eqref{eq:eig_max} yields $c(\gamma)$ as a
continuous function of $\gamma\geq\eta$
\begin{align}
\label{eq:condc}
c(\gamma) 
\coloneqq
...
\eqqcolon c_1(\gamma), 
\quad & \eta \leq \gamma \leq \eta + \tfrac{\beta^2}{\eta},
\\[3ex]
\displaystyle
...
\eta + \tfrac{\beta^2}{\eta}$, and plugging $\gamma = \gamma_*$ into
...
$|1 - 2\tfrac{\gamma-\eta}{\gamma}| < 1$. In addition, by \Cref{ass:fov} $W(\mathcal{L}_2)\leq 0$,
which implies $-\langle (\mathcal{L}_2+\mathcal{L}_2^T)\mathbf{w},\mathbf{w}\rangle \geq 0$
\todo{cite}. It follows that all terms in the numerator and denominator are
positive, and
%
...
performing direct solves, at a fraction of the cost.
...
Gauss decomposition applied to the simplified Newton setting after
