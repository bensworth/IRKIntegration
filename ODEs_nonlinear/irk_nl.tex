...
parallel solution of fully implicit Runge-Kutta schemes in the integration
of numerical PDEs, including nonlinearities and differential algebraic
equations (DAEs). This framework also naturally applies to discontinuous Galerkin
discretizations in time. The new method can be used with arbitrary existing
preconditioners for backward Euler-type time stepping schemes
...
Preconditioning \eqref{eq:dae_block} and the corresponding Schur complement
requires more problem-specific analysis than the general theory developed for
ODEs in \Cref{sec:theory}. In particular, \Cref{ass:fov} does not necessarily
hold for the larger linear system that includes time-dependent variables and
constraints (the obvious example being indefinite saddle-point systems that
often arise in incompressible fluid dynamics). However, \Cref{sec:numerics:khi}
considers a Picard iteration of incompressible Navier Stokes in
vorticity-stream-function form, where \eqref{eq:dae_block} can be reordered to
...
% ------------------------------------------------------------------------------------- %
% ------------------------------------------------------------------------------------- %
\subsection{Compressible Euler and Navier--Stokes equations}\label{sec:numerics:ns}
...
We discretize this problem using a high-order discontinuous Galerkin method with Roe numerical fluxes.
...
We compare the naive choice of $\gamma = \eta$ to the improved choice of $\gamma = \gamma_*$, where $\gamma_*$ is as in \Cref{th:cond_L1=L2}.
This choice is shown to be optimal in the case where $\cL_1 = \cL_2$.
Although this assumption does not hold in this case because the equations are fully nonlinear, we still observe significantly improved iteration counts with this choice of $\gamma$.
...
\begin{figure}
	...
	\fbox{\includegraphics[width=3in]{figures/naca_vorticity}}
...
The domain is discretized using a triangular mesh with 3154 elements, and the spatial discretization is a high-order discontinuous Galerkin method using compact stencils for the second order (viscous) terms with polynomial degree $p=3$.
...
For the IRK methods, one Kyrlov iteration for a $1\times1$ system corresponds to one preconditioner application, whereas for a $2\times2$ system, one Krylov iteration corresponds to two preconditioner applications.
...
% ------------------------------------------------------------------------------------- %
% ------------------------------------------------------------------------------------- %
\subsection{Incompressible Euler and Navier--Stokes in vorticity-streamfunction form}\label{sec:numerics:khi}

As an example of an index-1 differential-algebraic equation, we consider the vorticity-streamfunction formulation of the 2D incompressible Euler equations \cite{Liu2000}.
...
In the case of the Navier--Stokes equations, we discretize the viscous term on the right-hand side of \eqref{eq:ins-3} using a standard interior penalty discontinuous Galerkin method \cite{Arnold1982}.
...
In the notation of \Cref{sec:dae}, we have $\mathcal{L}_u^{(i)} = K(\psi^{(i)})$, $\mathcal{L}_w = 0$, $\mathcal{G}_u = M_{\rm mix}$, and $\mathcal{G}_w = A$.
Therefore, the $4\times4$ block system \eqref{eq:dae_block} that arises from IRK integration can be rearranged to be written as
\begin{align}
\begin{bmatrix}
	\eta M - \delta t\mathcal{L}_{u}^{(1)} & \phi M & \mathbf{0} & \mathbf{0} \\
	-\tfrac{\beta^2}{\phi}M & \eta M - \delta t\mathcal{L}_{u}^{(2)} & \mathbf{0} & \mathbf{0} \\
	-\delta t\mathcal{G}_{u} & \mathbf{0} & -\delta t\mathcal{G}_w & \mathbf{0} \\
	\mathbf{0} & -\delta t\mathcal{G}_{u}  & \mathbf{0} & -\delta t\mathcal{G}_w
\end{bmatrix}
\begin{bmatrix} \mathbf{k}_1 \\ \mathbf{k}_2 \\ \boldsymbol{\ell}_1 \\ \boldsymbol{\ell}_2 \end{bmatrix}
=
...
We consider the two-dimensional incompressible Taylor--Green vortex problem.
...
In order to test the accuracy of the time integration method, we instead consider the Navier--Stokes equations, for which the exact solution is given by
\[
