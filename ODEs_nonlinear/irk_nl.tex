...
% ------------------------------------------------------------------------------------- %
% ------------------------------------------------------------------------------------- %
\subsection{(Incompressible) Kelvin-Helmholtz instability}\label{sec:numerics_nonlin:khi}
...
Assume we have used the method of lines to discretize in space, and arrive at a coupled
set of nonlinear DAEs in time. In the context of a DAE \eqref{eq:dae}, we have nonlinear
function $F$ corresponding to $M\partial_t w - F = 0$, for mass matrix $M$, and linear
constraint $G$, where
%
\begin{align}\label{eq:FandG}
- F(w,\phi,\mathbf{v}) := \mathbf{v}\cdot\nabla w - \nabla_\perp \cdot\mu\nabla_\perp w, 
\hspace{5ex} G(w,\phi,\mathbf{v}) :=
	\begin{bmatrix} -\nabla_\perp^2\phi + w \\
		\mathbf{v} + \nabla \times \phi\hat{\mathbf{z}}
	\end{bmatrix}.
\end{align}
%
For DIRK schemes, where Butcher coefficients satisfy $a_{ij} = 0$ for $j > i$, each stage
vector is defined as in \eqref{eq:rk_dae}. The first stage vector
$\mathbf{k}_1 = [k_{w,1}; k_{\phi,1}; \mathbf{k}_{v,1}]$ is defined by the following
nonlinear set of equations,
%
\begin{align*}
\begin{bmatrix} Mw_n \\ 0 \\ 0\end{bmatrix} & = 
	\begin{bmatrix}
	Mk_{w,1} + a_{11}\delta t\left(\mathbf{k}_{v,1}\cdot\nabla k_{w,1} -
		\nabla_\perp \cdot\mu\nabla_\perp k_{w,1}\right) \\
	-\nabla_\perp^2 k_{\phi,1} + k_{w,1} \\
	\mathbf{k}_{v,1} + \nabla \times k_{\phi,1}\hat{\mathbf{z}}
	\end{bmatrix}.
\end{align*}
%
The stages $\mathbf{k}_i$, $i=2,...,s$ are given by the solution of a similar nonlinear
set of equations, with an updated right-hand-side based on previous stage vectors, 
%
\begin{align*}
\begin{bmatrix} Mw_n - a_{i1}\delta tF(\mathbf{k}_1) - ... - a_{i,i-1}\delta tF(\mathbf{k}_{i-1}) \\ 0 \\ 0\end{bmatrix} & = 
	\begin{bmatrix}
	Mk_{w,i} + a_{ii}\delta t\left(\mathbf{k}_{v,i}\cdot\nabla k_{w,i} -
		\nabla_\perp \cdot\mu\nabla_\perp k_{w,i}\right) \\
	-\nabla_\perp^2 k_{\phi,i} + k_{w,i} \\
	\mathbf{k}_{v,i} + \nabla \times k_{\phi,i}\hat{\mathbf{z}}
	\end{bmatrix}.
\end{align*}
%
Following from \eqref{eq:rk_dae2}, the updates to $\{w,\phi,\mathbf{v}\}$ are
then given by 
%
\begin{align*}
Mw_{n+1} & = Mw_n + \delta t \sum_{i=1}^s b_iF(\mathbf{k}_i), \\
-\nabla_\perp^2 \phi_{n+1} & = -w_{n+1}, \\
\mathbf{v}_{n+1} & = -\nabla\times \phi\hat{\mathbf{z}}.
\end{align*}
%

Fully implicit Runge-Kutta methods take a more complicated form, which is not
discussed here. Each stage above requires the solution of a nonlinear set of
equations, which we discuss the linearization and solution of below. 

% ------------------------------------------------------------------------------------- %
\subsubsection{Picard iteration}

The simplest linearization is a Picard iteration. Here, a Picard iteration takes a simple
form, where we linearize by defining $\mathbf{v}$ using the previous iterate of $\phi$.
For ease of notation, we will denote the stage vectors associated with $\{w,\phi,\mathbf{v}\}$
as such (rather than using $k$), because the equations for each stage are more-or-less
analogous. Formally, define the nonlinear matrix-valued operator
%
\begin{align*}
\mathcal{G}(w,\phi,\mathbf{v}) & =
	\begin{bmatrix}
	M + \zeta\delta t\left( \mathbf{v}\cdot\nabla  - \nabla_\perp \cdot\mu\nabla_\perp\right)
		& \mathbf{0} & \mathbf{0} \\
	I & -\nabla_\perp^2 & \mathbf{0} \\
	\mathbf{0} & \nabla \times \hat{\mathbf{z}} & I 
	\end{bmatrix}.
\end{align*}
%
Then, we seek the solution of $\mathcal{G}(\mathbf{u})\mathbf{u} - \mathbf{f} = \mathbf{0}$,
which we iterate towards using the Picard iterations $\mathbf{u}_{n+1} =
\mathcal{G}(\mathbf{u}_n)^{-1}\mathbf{f}$. In matrix form, each iteration requires the
solution of
%
\begin{align}\label{eq:Gsol1}
\begin{bmatrix}
	M + \zeta\delta t\left( \hat{\mathbf{v}}\cdot\nabla  - \nabla_\perp \cdot\mu\nabla_\perp\right)
		& \mathbf{0} & \mathbf{0} \\
	I & -\nabla_\perp^2 & \mathbf{0} \\
	\mathbf{0} & \nabla \times \hat{\mathbf{z}} & I 
\end{bmatrix}
\begin{bmatrix} {w} \\ \phi \\ \mathbf{v} \end{bmatrix}
- \begin{bmatrix} f \\ \mathbf{0} \\ \mathbf{0} \end{bmatrix}
= \mathbf{0},
\end{align}
%
where $\hat{\mathbf{v}}$ is the vector field $\mathbf{v}$ from the previous iteration. 
Note the inverse of \eqref{eq:Gsol1} can be done sequentially, and requires the solution of
the advection-diffusion operator, $Mw + \zeta\delta t( \hat{\mathbf{v}}\cdot\nabla w  -
\nabla_\perp \cdot\mu\nabla_\perp w) = f$, the anisotropic diffusion operator,
$-\nabla_\perp^2 \phi = {w}$, and an application of the curl operator, $\mathbf{v} =
-\nabla\times\hat{\mathbf{z}}\phi$. 
