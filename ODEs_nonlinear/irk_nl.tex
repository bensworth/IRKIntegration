...
techniques similar to the simplified Newton setting \Cref{sec:nonlinear:simp}, and
...
\end{align*}
%
for some $\gamma > 0$. \Cref{sec:theory:spd} considers the simplest case
...
$\widehat{S}_\gamma^{-1}$ exactly for each iteration of the preconditioner
...
\end{align}
%
The following theorem bounds the condition number of the preconditioned
Schur complement, applying a similar theoretical approach as used to analyze
preconditioners for IRK in the linear setting \todo{cite}. Although the
resulting conditioning here is slightly worse than can be achieved with the
...
%
\begin{theorem}[Optimal preconditioning, $\cL_1 = \cL_2$]\label{th:opt}
Let $\cL$ be real-valued and suppose \Cref{ass:eig,ass:fov} hold,
that is, $\eta > 0$ and $W(\cL) \leq 0$. Let $\mathcal{P}_\gamma$ denote
the preconditioned Schur complement \eqref{eq:P_gamma2}, where $S =
\eta I - \widehat{\mathcal{L}}_2 + \beta^2 (\eta I - \widehat{\mathcal{L}}_1)^{-1}$
\eqref{eq:Schur} is preconditioned with $(\gamma I - \cL)^{-2}$.
%
Let $\kappa({\cal P}_{\gamma})$ denote the two-norm condition number of
${\cal P}_{\gamma}$, and define $\gamma_*$ by 
\begin{align} \label{eq:gammastar}
\gamma_* \coloneqq \frac{\eta^2+\beta^2}{\eta}.
\end{align}
Then
\begin{align} \label{eq:P_gammastar}
\kappa(\mathcal{P}_{\gamma_*}) \leq 1+\frac{\beta^2}{2\eta^2}.
\end{align}

Moreover, (i) bound \eqref{eq:P_gammastar} is tight in the sense that $\exists$ $\cL$
that satisfy \eqref{eq:P_gammastar} exactly, and (ii) $\gamma = \gamma_*$ is optimal
in the sense that, without further assumptions on $\cL$, $\gamma_*$ minimizes a tight
upper bound on $\kappa({\cal P}_{\gamma})$ over all $\gamma \in (0, \infty)$.
\end{theorem}
\begin{proof}
...
The square of the condition number of ${\cal P}_{\gamma}$ is given by
%
\begin{align}
\label{eq:kappa_def}
...
to real-valued $\mathbf{v}$. The key step in establishing \eqref{eq:P_gammastar} is bounding
$\Vert {\cal P}_{\gamma} \Vert^2$ and $\Vert {\cal P}_{\gamma}^{-1} \Vert^2$ from above
by bounding $\Vert {\cal P}_{\gamma} \bm{v} \Vert^2 / \Vert \bm{v} \Vert^2$
from above and below, respectively.
%
Consider the form of the preconditioned Schur complement ${\cal P}_{\gamma}$ in
\eqref{eq:P_gamma} and make the substitution $\bm{v} = (\gamma I - \cL)^{2} \bm{w}$. 
Now, for real-valued $\bm{v}$ (and, thus, real-valued $\bm{w}$),
$\|{\cal P}_{\gamma} \bm{v}\|^2$ can be expanded as
%
\begin{align}
\label{eq:norm_Pv}
\begin{split}
\Vert {\cal P}_{\gamma} \bm{v} \Vert^2 &=
	\left\|\left[ (\eta^2+\beta^2) I - 2\eta \widehat{\mathcal{L}} +
		\widehat{\mathcal{L}}^2 \right]\mathbf{w}\right\|^2 \\
& = \| (\eta^2+\beta^2) \mathbf{w} + \widehat{\mathcal{L}}^2\mathbf{w}\|^2
	+ 4\eta^2\|\widehat{\mathcal{L}}\mathbf{w}\|^2 - 
	\\&\hspace{10ex}
	4\eta (\eta^2+\beta^2)\langle \widehat{\mathcal{L}}\mathbf{w},\mathbf{w}\rangle -
	4\eta \langle \widehat{\mathcal{L}}(\widehat{\mathcal{L}}\mathbf{w}),
		\widehat{\mathcal{L}}\mathbf{w}\rangle.
\end{split}
\end{align}
%
Similarly, expanding $\|\bm{w}\|^2$ yields
%
\begin{align}
\label{eq:norm_v}
\begin{split}
\Vert \bm{w} \Vert^2 &=
	...
	\widehat{\mathcal{L}}^2\right]\mathbf{w}\right\|^2 \\
&= \| \eta\gamma \mathbf{w} + \widehat{\mathcal{L}}^2\mathbf{w}\|^2
	+ (\eta+\gamma)^2\|\widehat{\mathcal{L}}\mathbf{w}\|^2 -
	\\&\hspace{10ex}
	2(\eta+\gamma)\eta\gamma\langle \widehat{\mathcal{L}}\mathbf{w},\mathbf{w}\rangle -
	2(\eta+\gamma) \langle \widehat{\mathcal{L}}(\widehat{\mathcal{L}}\mathbf{w}),
		\widehat{\mathcal{L}}\mathbf{w}\rangle.
\end{split}
\end{align}
%
Thus, the key term in \eqref{eq:kappa_def} takes the form
\begin{align}
\label{eq:P_gamma_frac}
\frac{\Vert {\cal P}_{\gamma} \bm{v} \Vert^2}{\Vert \bm{v} \Vert^2}
= \frac{ c_0(\bm{w}) f_0(\bm{w}) + c_1 f_1(\bm{w}) + c_2 f_2(\bm{w}) + c_3 f_3(\bm{w}) }
...
\begin{aligned}
\label{eq:f_c_def}
f_0 &\coloneqq \| \eta\gamma \mathbf{w} + \widehat{\mathcal{L}}^2\mathbf{w}\|^2 \ge 0,
\quad
&&c_0\coloneqq \frac{\| (\eta^2+\beta^2) \mathbf{w} + \widehat{\mathcal{L}}^2\mathbf{w}\|^2}
	{\| \eta\gamma \mathbf{w} + \widehat{\mathcal{L}}^2\mathbf{w}\|^2} \geq 0,
\\
f_1 &\coloneqq (\eta+\gamma)^2\|\widehat{\mathcal{L}}\mathbf{w}\|^2 \geq 0,
\quad 
&&c_1\coloneqq \frac{4\eta^2}{(\eta+\gamma)^2} > 0,\\
f_2 &\coloneqq -2\eta\gamma(\eta+\gamma)\langle \widehat{\mathcal{L}}\mathbf{w},\mathbf{w}\rangle \geq 0, 
\quad
&&c_2\coloneqq \frac{4\eta(\eta^2+\beta^2)}{2\eta\gamma(\eta+\gamma)} > 0, \\
f_3 &\coloneqq -2(\eta+\gamma) \langle \widehat{\mathcal{L}}(\widehat{\mathcal{L}}\mathbf{w}),
		\widehat{\mathcal{L}}\mathbf{w}\rangle \geq 0, 
\quad
&&c_3\coloneqq \frac{4\eta}{2(\eta+\gamma)} > 0,
\end{aligned}
\end{equation}
assuming $\gamma > 0$. Note that functions $f_2$ and $f_3$ are non-negative by
assumption of $W( \cL ) \leq 0$, while for all $\mathbf{w}$, it must hold that
either $f_1 > 0$ or $c_0f_0 > 0$ (or both).
...
appears in expanding $\| \eta\gamma \bm{w} + \cLs \bm{w} \|^2$) is not
known for general $\cL$, noting that the sign of $W(\cL)$ does not determine
that of $W(\cLs)$. However, observe from \eqref{eq:f_c_def} that the judicious
choice of $\gamma = \gamma_* \coloneqq (\eta^2 + \beta^2)/\eta$ yields
$c_0(\bm{w}) = 1$. 
Letting $\gamma = \gamma_*$, \eqref{eq:f_c_def} yields $1 = c_0 > c_2 = c_3 >
c_1 = \left(\frac{2\eta^2}{2\eta^2+\beta^2}\right)^2$. Thus, for $\gamma = \gamma_*$,
the bounds in \eqref{eq:P_gamma_bounds} are given by
\begin{align}
\label{eq:P_gamma*_bounds}
\Vert {\cal P}_{\gamma_*} \Vert^2 \leq 1,
\quad
\Vert {\cal P}_{\gamma_*}^{-1} \Vert^2 \leq \left( 1 + \frac{\beta^2}{2\eta^2}\right)^2 .
\end{align}
Applying these bounds to the condition number \eqref{eq:kappa_def}
yields the upper bound in \eqref{eq:P_gammastar}.

\tcb{Finish this part}

We now show that bound \eqref{eq:P_gammastar} is tight and that
$\gamma = \gamma_*$ is optimal. We do so by construction, showing that the
bound in \eqref{eq:P_gamma_bounds} is achieved for certain matrices, and 
that for $\gamma\neq\gamma_*$, $\exists$ matrices $\cL$ for which
$\kappa({\cal P}_{\gamma}) > \sqrt{1 + \beta^2/\eta^2}$. Note that the min/max 
of $\Vert {\cal P}_{\gamma} \bm{v} \Vert^2/\Vert \bm{v} \Vert^2$ over $\mathbf{v}$
for real-valued ${\cal P}_{\gamma}$ is equivalent when minimizing over real
or complex $\bm{v}$; we now consider complex $\mathbf{v}$ for theoretical
purposes. To that end, let $~\bm{v}=(\gamma I - \cL)^2 \bm{w}$, but 
suppose that $(\textrm{i} \xi, \bm{w})$ is an eigenpair of $\cL$, with $\xi$
a real number and $\bm{w}$ a complex eigenvector. Plugging into
$\| {\cal P}_{\gamma} \bm{v}\|^2$ \eqref{eq:norm_Pv} and $\|\bm{v}\|^2$
\eqref{eq:norm_v}, and taking the ratio as in \eqref{eq:P_gamma_frac},
...
\end{align}
%
That is, any value of $1/{\cal H}_{\gamma}(\xi)$ serves as a lower bound on
$\Vert {\cal P}_{\gamma}^{-1} \Vert^2$, while any value of ${\cal
H}_{\gamma}(\xi)$ serves as a lower bound on $\Vert {\cal P}_{\gamma} \Vert^2$.
Therefore, the ratio of any two values of ${\cal H}_{\gamma}(\xi)$ provide a
lower bound on $\kappa^2({\cal P}_{\gamma})$.

We now show that bound \eqref{eq:P_gammastar} on $\kappa^2({\cal P}_{\gamma_*})$
is tight. Considering \eqref{eq:H_def} at the judiciously chosen eigenvalues of
$\mathrm{i} \xi = \{ 0, \pm \mathrm{i} \gamma_*\}$, we have
%
\begin{align}
\label{eq:H1}
{\cal H}_{\gamma}(0) 
...
\end{align}
%
First observe from \eqref{eq:H_bounds} and \eqref{eq:H1} that $\Vert {\cal
P}_{\gamma_*} \Vert^2 \geq {\cal H}_{\gamma_*}(0) = 1$, and thus the upper bound
on $\Vert {\cal P}_{\gamma} \Vert^2$ from \eqref{eq:P_gamma*_bounds} achieves
equality for a matrix $\cL$ having an eigenvalue of $\xi = 0$.
Secondly, observe from \eqref{eq:H_bounds} and \eqref{eq:H1} that $\Vert {\cal
P}_{\gamma_*}^{-1} \Vert^2 \geq \tfrac{1}{{\cal H}_{\gamma_*}(\pm \gamma_*)}= 1
+ \tfrac{\beta^2}{\eta^2}$, and thus the upper bound on $\Vert {\cal
P}_{\gamma}^{-1} \Vert^2$ from \eqref{eq:P_gamma*_bounds} achieves equality for
a matrix $\cL$ having eigenvalues $\mathrm{i} \xi = \pm \mathrm{i} \gamma_*$.
Therefore, bound \eqref{eq:P_gammastar} on $\kappa({\cal P}_{\gamma_*})$
achieves equality for any matrix $\cL$ having eigenvalues $\{0, \pm
\mathrm{i}\gamma_*\}$.

Finally, having shown that \eqref{eq:P_gammastar} is tight, we now show that 
$\gamma = \gamma_*$ is optimal. Once again, consider the matrix $\cL$ from above
with eigenvalues $\{0, \pm \mathrm{i}\gamma_* \}$, such that
$\kappa^2({\cal P}_{\gamma_*})
=
1 + \frac{\beta^2}{\eta^2}$. Now, observe from this, \eqref{eq:H_bounds},
and \eqref{eq:H1} that for $0 < \gamma < \gamma_*$,
%
\begin{align}
...
which, when substituted into \eqref{eq:H_def}, yields
$\lim_{\xi \to \pm \infty} {\cal H}_{\gamma}(\xi) = 1$.
Combining with \eqref{eq:H_bounds} and \eqref{eq:H1}, we have 
...
By construction in \eqref{eq:kappa_lwr_bnd1} and \eqref{eq:kappa_lwr_bnd2},
we have shown that for all $\gamma \in (0, \infty) \setminus \gamma_*$, there exists
matrices $\cL$ such that $\kappa({\cal P}_{\gamma}) > \kappa({\cal
P}_{\gamma_*}) = \sqrt{1 + \beta^2/\eta^2}$. It therefore holds for general $\cL$
satisfying \Cref{ass:eig,ass:fov} that a tight upper bound on $\kappa({\cal P}_{\gamma})$
for $\gamma \in (0, \infty) \setminus \gamma_*$ must be larger than the tight upper
bound of $\kappa({\cal P}_{\gamma_*}) \leq \sqrt{1 + \beta^2/\eta^2}$. Hence $\gamma=\gamma_*$ is the
minimizer over $\gamma \in (0, \infty)$ of a tight upper bound on $\kappa({\cal
P}_{\gamma})$.
\end{proof}
%

\Cref{tab:cond} provides condition number bounds from \Cref{th:opt} and
...
Lobatto IIIC & 1.50 & 1.00 & 2.11 & 2.76 & 1.07 & 1.00 & 3.44 & 1.19 \\\hline
  \end{tabular}
  \caption{Bounds on $\kappa(\mathcal{P}_{\gamma_*})$ from \Cref{th:opt} and
...
%
\begin{remark}[SPD and skew-symmetric operators]
\tcb{Fill in with bounds from SPD and SS analysis.}
...
\Cref{th:opt}, \Cref{th:cond} derives an upper bound on condition number of the
right-preconditioned Schur complement as in \eqref{eq:P_gamma}, with $\gamma_*$
as in \eqref{eq:gammastar}. Obtaining accurate bounds requires some assumption
...
simply that $\cL_1$ and $\cL_2$ are not wildly different operators.
...
$\cL_1 = \cL_2$ in \Cref{th:opt}. By \Cref{tab:cond}, it is clear the
...
and define $\gamma_* := \tfrac{\eta^2+\beta^2}{\eta}$ as in \eqref{eq:gammastar}.
Then
...
\end{theorem}
\begin{proof}
As in \Cref{th:opt}, the square of the condition number of ${\cal P}_{\gamma}$
is given by
%
\begin{align}
\label{eq:kappa_def2}
...
:= \tfrac{\eta^2+\beta^2}{\eta}$ \eqref{eq:gammastar}. Expanding,
%
{\small
\begin{align}
& \hspace{-5ex}
...
a similar technique as in the proof of \Cref{th:opt}, wherein we consider the maximum
