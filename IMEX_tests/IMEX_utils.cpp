...
void IMEXEuler2::Init(IRKOperator &_imex)
{
    ODESolver::Init(_imex);
    imex = &_imex;
    k.SetSize(imex->Width(), mem_type);
...
void IMEXEuler2::Step(Vector &x, double &t, double &dt)
{
    // 121 scheme from Ascher et al.
    //      u_1 = u_{n-1} + dtM^{-1}Ni(u1) + dtM^{-1}Ne(u_{n-1}),
    //      u_n = u_{n-1} + dtM^{-1}Ni(u1) + dtM^{-1}Ne(u_{1})
    // Split tableaux:
    //  0 | 0 0    0 | 0 0
    //  1 | 0 1    1 | 1 0
    // ---+-----  ---+-----
    ...
    imex->MassMult(x, k);           // k = Mu_{n-1}
    imex->SetTime(t);
    imex->ExplicitMult(x, z);       // z = Ne(u_{n-1})
    k.Add(dt, z);                   // k = Mu_{n-1} + dtNe(u_{n-1})
    imex->SetTime(t + dt);
    // Solve (u1 - dtM^{-1}Ni(u1)) = u_{n-1} + dtM^{-1}Ne(u_{n-1})
    // <--> (M*u1 - dtNi(u1)) = Mu_{n-1} + dtNe(u_{n-1})
    // <--> (M*u1 - dtNi(u1)) = k
    imex->ImplicitSolve2(dt, k, u1);

    imex->ExplicitMult(u1, k);      // k = Ne(u1)
    imex->ImplicitMult(u1, z);      // z = Ni(u1)
    z += k;                         // z = Ne(u1) + Ni(u1)
    z *= dt;                        // z = dt*Ne(u1) + dt*Ni(u1)
    imex->MassInv(z, u1);           // u1 =  dt*M^{-1}Ne(u1) + dt*M^{-1}Ni(u1)
    x += u1;                        // u_{n+1} = u_n + dt*M^{-1}Ne(u1) + dt*M^{-1}Ni(u1)
    t += dt;
}


// ImplicitSolve solves for k = f(x+dt*k, t);
//
IMEXRK222::IMEXRK222(bool indirectExp_) :  ODESolver(),
    indirectExp(indirectExp_), have_stage1(false), previous_dt(-1)
{

}

void IMEXRK222::Init(IRKOperator &_imex)
{
    ODESolver::Init(_imex);
    imex = &_imex;
    ke1.SetSize(imex->Width(), mem_type);
    ke2.SetSize(imex->Width(), mem_type);
    k1.SetSize(imex->Width(), mem_type);
    k2.SetSize(imex->Width(), mem_type);
}


// TODO : do I need to set time of implicit operator?
void IMEXRK222::Step(Vector &x, double &t, double &dt)
{
    //   Explicit            Implicit
    //  0 | 0   0   0      0 | 0   0   0
    //  g | g   0   0      g | 0   g   0
    ...
    //    | d  1-d  0        | 0  1-g  g
    //
    // for g = 1-sqrt(2)/2, d = -2sqrt(2)/3
    // const double g = 1.0 - sqrt(2.0) / 2.0;
    // const double d = -2.0 * sqrt(2.0) / 3.0;
    const double g = (2.0 - std::sqrt(2.0)) / 2.0;
    const double d = 1 - 1.0 / (2 * g);


    // For first time step, need to directly compute the action
    // of the explicit part of the operator
    if (!have_stage1 || dt != previous_dt)
    {
        imex->SetTime(t);
        imex->ExplicitMult(x, ke2);
        imex->MassInv(ke2, ke1);
        have_stage1 = true;
        previous_dt = dt;
    }

    // Solve first implicit stage
    imex->SetTime(t + dt*g);
    x.Add(dt*g, ke1);
    k1 = 0.0;
    imex->ImplicitSolve(dt*g, x, k1);

    // Apply second explicit stage
    x.Add(dt*g, k1);
    if (indirectExp) {
        imex->Mult(x, k2);
        imex->MassInv(k2, ke2);
        ...
        imex->ExplicitMult(x, k2);
        imex->MassInv(k2, ke2);
    }

    // Solve second implicit stage
    // Redefine x as new sum over stages, subtracting previous entries
    // x += (dt * (d - g) * ke1 + dt * (1 - 2 * g) * k1 + dt * (1 - d) * ke2);
    imex->SetTime(t + dt);
    x.Add(dt*(d - g), ke1);
    x.Add(dt*(1-2*g), k1);
    x.Add(dt*(1-d), ke2);
    k2 = 0.0;
    imex->ImplicitSolve(dt * g, x, k2);

    // Compute new solution and optionally first stage for next time step
    x.Add(dt*g, k2);
    if (indirectExp) {
        imex->Mult(x, k1);
        imex->MassInv(k1, ke1);
...
void IMEXRK232::Init(IRKOperator &_imex)
{
    ODESolver::Init(_imex);
    imex = &_imex;
    ke1.SetSize(imex->Width(), mem_type);
    ke2.SetSize(imex->Width(), mem_type);
    ke3.SetSize(imex->Width(), mem_type);
    k1.SetSize(imex->Width(), mem_type);
    k2.SetSize(imex->Width(), mem_type);
}
...
// TODO : do I need to set time of implicit operator?
void IMEXRK232::Step(Vector &x, double &t, double &dt)
{
    //   Explicit            Implicit
    //  0 | 0   0   0      0 | 0   0   0
    //  g | g   0   0      g | 0   g   0
    ...
    // Compute final explicit stage
    x.Add(dt*g, k2);
    Vector temp(x.Size());
    imex->ExplicitMult(x, temp);
    imex->MassInv(temp, ke3);

    // Add stages for updated solution (stiffly accurate implicit -> already accounted for).
    // TODO : this can save in allocation.
    x.Add(-dt*d, ke1);
    x.Add(dt*((1-g) - (1-d)), ke2);
    x.Add(dt*g, ke3);
...
IMEXRK::~IMEXRK()
{
    for (int i = 0; i < tableaux.s; i++) {
        if (imp_stages[i]) delete imp_stages[i];
        if (exp_stages[i]) delete exp_stages[i];
    }
}


void IMEXRK::Init(IRKOperator &_imex)
{
    ODESolver::Init(_imex);
    imex = &_imex;
    ...
    *exp_stages[0] = 0.0;
    ...
    // TODO : only allocate last explicit stage if necessary
...
void IMEXRK::Step(Vector &x, double &t, double &dt)
{
    ...
    imex->SetTime(t);
    Vector temp(x.Size());      // TODO : can we avoid allocating this vector?? Used later to.
    imex->ExplicitMult(x, temp);
    ...
    for (int i = 1; i < s; i++) {
        ...
        for (int j = 0; j < i; j++) {
            ...
            if (std::abs(c0) > 1e-15) {
                ...
                if (debug) std::cout << "Modifying implicit ind " << j << ", " << c0 << "\n";
            }
        }
        ...
        for (int j = 0; j < (i - 1); j++) {
            ...
            if (std::abs(c0) > 1e-15) {
                ...
                if (debug) std::cout << "Modifying explicit ind " << j << ", " << c0 << "\n";
            }
        }
        // x += dt*Ae_{i,i-1}*\hat{k}_{i}
        if (std::abs(tableaux.Ae(i, i - 1)) > 1e-15) {
            x.Add(dt * tableaux.Ae(i, i - 1), (*exp_stages[i - 1]) );
            if (debug) std::cout << "Adding explicit ind " << i - 1 << ", " << dt*tableaux.Ai(i, i) << "\n";
        }
        ...
        if (std::abs(tableaux.Ai(i, i)) > 1e-15) {
            ...
            if (debug) std::cout << "Adding implicit ind " << i << ", " << dt*tableaux.Ai(i, i) << "\n";
        }
    ...
    if (!tableaux.stiffly_accurate) {
        ...
        if (std::abs(tableaux.be(s - 1)) > 1e-15) {
            ...
            if (debug) std::cout << "Adding final explicit ind " << s - 1 << ", " << dt*tableaux.be(s - 1) << "\n";
        }
    }
    t += dt;
}
...
void IMEXBDF::Init(IRKOperator &_imex)
{
...
// TODO : check on explicitMult and MassInv, make sure are doing
// right thing and consistent with RK code
void IMEXBDF::Step(Vector &x, double &t, double &dt)
{
    ...
    // Typical BDF time steps once we have sufficient starting values
        ...
        if (data.shifted_nodes) {
            ...
            if (!recompute_exp || data.q==1) ClassicalStep(x, t, r);
            else ClassicalStepNoStore(x, t, r);
        }
...
// I think during this step we have to store 3q+2 vectors; traditional BDF
// only requires 2q+2 vectors w/o computing explicit components, or q+3 when
// recomputing explicit components.
void IMEXBDF::AlphaStep(Vector &x, double &t, double &r)
{
    ...
    // Apply mass matrix to stored solution values
    {
    ...
    // of operator evaluated at previous solution values
    ...
    for (int i = 0; i < (data.q - 1); i++) {
        ...
        for (int j = 0; j < data.q; j++) {
            if (i == j) continue;   // Already accounted for this one.
        ...
        // Apply mass inverse, swap pointers so result is stored
        // in sols[i]
...
// previous solution values as well
