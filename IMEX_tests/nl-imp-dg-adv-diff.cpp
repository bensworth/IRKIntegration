...
double eta = 1e-2;    // reaction coefficient
...
double force_fn(const Vector &xvec, double t)
{
   ...
   // double u0 = sin(2*M_PI*x*(1-y)*(1+2*t))*sin(2*M_PI*y*(1-x)*(1+2*t));
   // double r = eta*u0*(1-u0)*(u0-0.5);
   // return -v - r;
   ...
   double v = 4.0*(M_PI+2.0*M_PI*t)*(M_PI+2.0*M_PI*t)*eps*(cos(2.0*M_PI*(1+2.0*t)*(x+(-1)*y))+((-1)+ ...
      ...
      +x+y)*(-1)*sin(2.0*M_PI*(1+2.0*t)*((-1)*y+x*((-1)+2.0*y)));
   return v;
...
public:
        ...
        dt_(-1)
   {
      ...
      b_imp.AddDomainIntegrator(new DomainLFIntegrator(forcing_coeff));
   ...
   void SetReaction(const Vector &u) const
   {
      ...
      for (int i=0; i<reaction.Size(); i++) {
         ...
         if (std::abs(reaction(i)) > 100) {
            if (root) std::cout << "Large reaction = " << reaction(i) << "\n";
         }
      }
   }
