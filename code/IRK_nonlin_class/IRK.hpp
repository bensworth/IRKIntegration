...
void KronTransformTranspose(const DenseMatrix &A, const BlockVector &x,
                            BlockVector &y);
...
public:
   ...
   DenseMatrix
   R0;     // Quasi-upper triangular matrix in Schur decomposition of A0^-1
...
public:
   ...
   {
      MFEM_ASSERT(u,
                  "IRKStageOper::Mult() Requires states to be set, see SetParameters()");
...
private:
   ...
   KronJacSolver *
   m_jacobian_solver;  // Linear solver for inverting approximate Jacobian
   NewtonParams m_newton_params;       // Parameters for Newton solver
   KrylovParams m_krylov_params;       // Parameters for Krylov solver.
   KrylovParams
   m_krylov_params2;      // Parameters for Krylov solver for 2x2 systems, if different than that for 1x1 systems
   ...
   vector<int>
   m_avg_krylov_iter;  // Avg number of Krylov iterations per time step (for every 1x1 and 2x2 system)
...
public:
   ...
   {
      MFEM_ASSERT(!m_solversInit,
                  "IRK::SetNewtonParams:: Can only be called before IRK::Run()");
   ...
   {
      MFEM_ASSERT(!m_solversInit,
                  "IRK::SetKrylovParams:: Can only be called before IRK::Run()");
...
public:
   ...
   {
      MFEM_ASSERT(N_jac,
                  "KronJacDiagBlock::Mult() must set Jacobian block with SetParameters()");
      MFEM_ASSERT(x.Size() == this->Height(),
                  "KronJacDiagBlock::Mult() incorrect input Vector size");
      MFEM_ASSERT(y.Size() == this->Height(),
                  "KronJacDiagBlock::Mult() incorrect output Vector size");
...
public:
   ...
   KronJacSolver(IRKStageOper &StageOper_,
                 const IRK::KrylovParams &solver_params1,
                 const IRK::KrylovParams &solver_params2,
      ...
      : Solver(StageOper_.Height()),
        ...
        y_2block(), z_2block(),
        temp_scalar1(StageOper_.RowOffsets()[1]),
        temp_scalar2(StageOper_.RowOffsets()[1]),
        ...
        N_jac_update_rate{N_jac_update_rate_}
   {
      ...
      for (int i = 0; i < StageOper.Butcher.s_eff; i++)
      {
         ...
         if (StageOper.Butcher.R0_block_sizes[i] == 1)
         {
            ...
            JacDiagBlockPrec[i] = new KronJacDiagBlockPrec(offsets[1], *(StageOper.IRKOper),
                                                           identity);
         }
         // 2x2 diagonal block
         else
         {
            ...
            JacDiagBlock[i] = new KronJacDiagBlock(2*offsets[1], *(StageOper.IRKOper), R,
                                                   offsets_2);
            JacDiagBlockPrec[i] = new KronJacDiagBlockPrec(2*offsets[1],
                                                           *(StageOper.IRKOper),
      ...
      krylov_solver2 =
         krylov_solver1; // By default, 2x2 systems solved with krylov_solver1.
      ...
      if ((type1_solves && type2_solves) && (&solver_params1 != &solver_params2))
      {
         MFEM_ASSERT(solver_params2.solver == IRK::KrylovMethod::GMRES,
                     "IRK:: 2x2 systems must use GMRES.\n");
   ...
   inline void GetKrylovSolver(IterativeSolver * &solver,
                               const IRK::KrylovParams &params)
   {
   ...
   void SetOperator (const Operator &op)
   {
      ...
      if (StageOper.getGradientCalls == 1 || N_jac_update_rate == 1)
      {
         ...
         {
            int idx = N_jac_lin-1;
            add(*(StageOper.u), dt, StageOper.GetCurrentIterate().GetBlock(idx),
                temp_scalar1);
