     ...
     m_comm{IRKOper_->GetComm()}
{
   ...
   for (int i = 0; i < m_Butcher.s_eff; i++)
   {
      ...
      else
      {
         ...
         m_eig_ratio[i] = sqrt(-m_Butcher.R0(row+1,row)*m_Butcher.R0(row,
                                                                     row+1)) / m_Butcher.R0(row,row);
         row++;
      }
      row++;
   }
...
void IRK::SetSolvers()
{
   ...
   int N_jac_lin =
      m_Butcher.s;// Linearize so that Jacobian is evaluated correct for last stage
   ...
   m_nonlinear_solver->SetSolver(
      *m_jacobian_solver); // The linear solver for the Newton method
}
...
void IRK::Step(Vector &x, double &t, double &dt)
{
   ...
   {
      string msg = "IRK::Step() Newton solver at t=" + to_string(
                      t) + " not converged\n";
...
void IRK::Run(Vector &x, double &t, double &dt, double tf)
{
   ...
   {
      step++;
      mfem::out << "Time-step " << step << " of " << numsteps << " (t=" << t <<
                "-->t=" << t+dt << ")\n";
