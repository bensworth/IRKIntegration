...
public:
    ...
    /* ---------------------------------------------------------------------- */
    /* ---------------- Virtual functions for IRK Multigrid ----------------- */
    /* ---------------------------------------------------------------------- */

    virtual void SetCoarseIRKOperator(double dt, double s_min, const Vector &L_coefficients)
    {
        mfem_error("IRKOperator::SetCoarseIRKOperator() is not overridden!");
    }

    virtual void CoarseIRKSolve(const Vector &x, Vector &y) const
    {
        mfem_error("IRKOperator::CoarseIRKSolve() is not overridden!");
    }
...
public:
    ...
    {        
        ...
        // Ensure that valid Z_solver and Z_prec provided for non Kronecker Jacobian
        if (!kronecker_form) {
            MFEM_ASSERT((Z_solver) && (Z_prec), "TriJacRelax:: IRKOperator using \
            exact gradients requires non NULL sparsity patterns Z_solver and Z_prec");
        }
        ...
        for (int block = 0; block < s_eff; block++) {
            ...
            // 2x2 diagonal block spanning rows=(row,row+1),cols=(row,row+1)
                ...
                mfem_error("TriJacRelax:: R0 block sizes must be 1 or 2");
            }
...
/* TODO:
    - Add interp/restrict arrays to RKData class with smallest sing vecs
...
public:
        ...
        jac_update_rate(jac_update_rate_), s_min(StageOper.Butcher.smin)
    {
        ...
        if (StageOper.Butcher.ID < 10) {
            MFEM_ABORT("Multigrid only for fully implicit RK, not DIRK.\n");
        }
        if (StageOper.Butcher.s < 2) {
            MFEM_ABORT("Multigrid only for > 1 implicit stages.\n");
        }
    ...
    {
        // Ensure coarse operator is built and up to date.
        double dt = StageOper.GetTimeStep();
        StageOper.IRKOper->SetCoarseIRKOperator(dt, s_min, L_coefficients);

        if (x.Size() != width) {
            MFEM_ABORT("GMRES vector size != Fine matrix size!\n");
        }
        ...
        // zfine = PK^{-1}R*zfine = PK^{-1}R(I - AM^{-1})*x
        xcoarse.SetSize(coarse_width);
        ycoarse.SetSize(coarse_width);
        Restrict(zfine, xcoarse);
        StageOper.IRKOper->CoarseIRKSolve(xcoarse, ycoarse);
        Interpolate(ycoarse, zfine);

        // Two-level multigrid as preconditioner,
