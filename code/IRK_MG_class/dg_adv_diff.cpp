...
public:
   ...
   /** Ensures that this->ImplicitPrec() preconditions (\s_min*M - dt*sum(L_coefficients)*L)
         + dt    -> time step size
         + s_min -> constant multiplied by mass matrix
         + L_coefficients -> precondition with L = \sum_i L_coefficients[i]*L_i
      Here, Li = Lj for all i,j, so this just ends up being a sum over L_coefficients.
   */
   virtual void SetCoarseIRKOperator(double dt, double s_min, const Vector &L_coefficients) override
   {
      if (!current_prec)
      {
         ...
         // Form preconditioner
         if (use_AIR > 0) {
            current_prec = new BackwardEulerPreconditioner(fes, s_min, *M_mat, shift,
