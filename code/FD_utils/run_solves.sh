...
dir=data/"$time_type"
out=IRK"$IRK"_dt^REFINE^_d"$dim"_FD"$FD"
...
do
    ...
    mpirun -np $np ../IRK_class/driver -t $IRK -dt $dt -tf $tf -o $space -l $(($dt_refine+2)) -d $dim -FD $FD -atol $atol -rtol $rtol -maxit 250 -kdim 30 -save $save -out $dir${out/^REFINE^/$dt_refine} 
