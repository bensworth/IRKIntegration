...
block_prec_ID=1 # Lower triangular Gauss--Seidel
#block_prec_ID=3 # Optimized lower triangular from Staff et al. (2006)
#block_prec_ID=4 # Rana et al. (2021)
...
out=PREC"$block_prec_ID"_IRK"$IRK"_h^REFINE^_d"$dim"_ex"$ex" 
...
do
    ...
    # Run with dt = 2*dx
    mpirun -np $np $exe \
        -irk $IRK_alg -prec_ID $block_prec_ID \
        -t $IRK -dt $dt -tf $tf \
        -o $space -l $h_refine \
        -d $dim -ex $ex \
