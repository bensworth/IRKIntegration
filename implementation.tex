...
\todo{Go through and explicitly write out mass matrix so it's clear for the implementation where there need to be solves and multiplies etc.}

The basic idea is that at each time step, we need to update the previous solution with
\begin{align} \label{eq:RK_update}
\bm{u}_{n+1}  = \bm{u}_n + \delta t \mdet({\cal M}_s)^{-1} \big( \bm{d}^\top_0 \otimes I \big) \madj({\cal M}_s) \bm{f},
\end{align}
where we have
\begin{itemize}
...
\bm{z} = \big( \bm{d}^\top_0 \otimes I \big) \madj ({\cal M}_s) \bm{f} \in \mathbb{R}^m
\end{align}
...
$\madj ({\cal M}_s)$ is defined by evaluating the adjugate of $A_0^{-1} - zI$ at $z = {\cal L}$, which is a matrix over polynomials of degree $s-1$ in $z$. If we write such polynomials as $Q_{ij}(z) \coloneqq \madj(A_0^{-1} - zI)_{ij}$, then
\begin{align}
\madj ({\cal M}_s) = 
\begin{bmatrix}
...
Now, the rectangular Kronecker product matrix appearing in front of this matrix simply takes inner products over its columns to give a block rectangular matrix whose elements are therefore polynomials in ${\cal L}$ of degree $s-1$:
\begin{align}
...
As shown previously, 
\begin{align}
\mdet ({\cal M}_s) = P_s({\cal L}) = \prod \limits_{j = 1}^s (\lambda_j I - {\cal L}),
\end{align}
...
P_s ({\cal L}) 
&= 
\prod \limits_{j = 1}^{s_{\Re}} (\zeta_j I - {\cal L}) \prod \limits_{j = 1}^{s_{\Im}} \big( [\eta_j^2 + \beta_j^2] I - 2 \eta_j {\cal L} + {\cal L}^2 \big),
\\
&\equiv
\prod \limits_{j = 1}^{s_{\Re}} {\cal R}_j({\cal L}) \prod \limits_{j = 1}^{s_{\Im}} {\cal I}_j({\cal L}).
\end{align}
...
\begin{enumerate}
\item To invert ${\cal R}_j({\cal L})$, we use a Krylov solver preconditioned with AMG applied to ${\cal R}_j({\cal L})$

\item To invert ${\cal I}_j({\cal L})$, we use a Krylov solver preconditioned with the operator $(\eta_j I - {\cal L})^2$, which we only approximately invert by \textbf{twice} applying AMG to $(\eta_j I - {\cal L})$. \tcp{Ben: So why cant we approximately invert $(\eta_j I - {\cal L})^2$ by 2 $\times$ [GMRES on $(\eta_j I - {\cal L})$ preconditioned by AMG on $(\eta_j I - {\cal L})$]. You mentioned in the code I'd written that this doesn't work. My thinking was that we need to approximately invert $(\eta_j I - {\cal L})$ and GMRES preconditioned with AMG will do this more robustly than standalone AMG?}
