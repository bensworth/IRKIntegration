...
\tcb{So we can't scale by $M^2$ and get an equivalent system because $M$ and $\mathcal{L}$ don't commute.
I think the best we can do here is to scale by $M$ to arrive at the system
\[
[ (\eta_j^2 + \beta_j^2)M - 2 \eta_j \delta t {\cal L} + \delta t^2{\cal L}M^{-1}{\cal L}] \bm{y}_j
= 
M \bm{z}_j
\]
So now instead of two applications of $M$ and $M^{-1}$ in each application of the preconditioning
and operator, we just have one. Similar to above, if ${\cal L}$ is symmetric, now the quadratic
operator is symmetric as well, and we can apply CG/MINRES. I think this is probably the ``right''
approach and what we should try to implement. The only catch is a simple algorithm to apply the
scaled operator
\[
 (\eta_j^2 + \beta_j^2)M - 2 \eta_j \delta t {\cal L} + \delta t^2{\cal L}M^{-1}{\cal L}
\]
using calls to user-provided functions (presumably the application of $M$, $\mathcal{L}$, and
$M^{-1}\mathcal{L}$).
}
\end{enumerate}
