...
\newpage
\newpage
\section{Numerical results II}

\begin{itemize}
\setlength\itemsep{0.5em}

\item Want to look at cost and scalability of solvers. Kind of hard to decide what  to look at, there are many different parameters...

\item Look at two examples:
\begin{enumerate}
\item SDIRK4: Has 5 linear factors.

\item Gauss8: Has 2 quadratic factors. Efficiency of preconditioned Krylov solves is dependent on the ratio $\beta/\eta$, with smaller $\beta/\eta$ leading to a faster solve
\end{enumerate}
...
\begin{figure}[H]
\centerline{
\includegraphics[width = 0.55\textwidth]{figures/SDIRKc}
\quad
\includegraphics[width = 0.55\textwidth]{figures/SDIRK_gmres}
}
\caption{SDIRK4+U4: Performance as a function of $\delta t$ and $\delta t / \delta x$; note that increasing $\delta t/\delta x$ means increasing the size of the spatial problem. \textbf{Left:} Discretization error. \textbf{Right:} Number of GMRES iterations.
\label{fig:SDIRK4}
}
\end{figure}
...
\begin{figure}[htb!]
\centerline{
\includegraphics[width = 0.45\textwidth]{figures/Gauss8c}
}
\centerline{
\includegraphics[width = 0.55\textwidth]{figures/Gauss8_gmres}
\quad
\includegraphics[width = 0.55\textwidth]{figures/Gauss8_gmres_5iters}
}
\caption{Gauss8+U8: Performance as a function of $\delta t$ and $\delta t / \delta x$; note that increasing $\delta t/\delta x$ means increasing the size of the spatial problem. \textbf{Top:} Discretization error. \textbf{Left/Right:} Number of GMRES iterations using \underline{two/ten} applications of AMG as a preconditioner.
\label{fig:Gauss8}
}
\end{figure}
...
Let's write the Jacobian of ${\cal N}$ w.r.t. the stages as
\begin{align}
J_{\cal N} = 
\delta t (A \otimes I)
\begin{bmatrix}
{\cal N}' (\bm{u}_n + \delta t \sum \limits_{j = 1}^s \bm{k})
\end{bmatrix}
\end{align}
