...
virtual void IRKOperator::ExplicitGradientMult(int index, const Vector &x,
    Vector &y) const
{
    MFEM_ASSERT(m_gradients == ExplicitGradients::EXACT, 
                "IRKOperator::ExplicitGradientMult() applies only for \
...
virtual void IRKOperator::SetPreconditioner(int index, double dt,
    double gamma, Vector weights) 
{
    mfem_error("IRKOperator::SetPreconditioner() is not overridden!");
}
...
JacDiagBlock::JacDiagBlock(const Array<int> &offsets_, const IRKOperator &IRKOper_,
    double R00_)
    : BlockOperator(offsets_),
    size(1), offsets(offsets_), IRKOper(IRKOper_),
    dt(0.0), temp_vector(IRKOper_.Height()),
    R00(R00_)
{
...
JacDiagBlock::JacDiagBlock(const Array<int> &offsets_, const IRKOperator &IRKOper_,
    ...
    : BlockOperator(offsets_),
    size(2), offsets(offsets_), IRKOper(IRKOper_),
    dt(0.0), temp_vector(IRKOper_.Height()),
...
JacDiagBlock::JacDiagBlock(const Array<int> &offsets_, const IRKOperator &IRKOper_,
    ...
    : BlockOperator(offsets_),
    size(2), offsets(offsets_), IRKOper(IRKOper_), dt(0.0),
    temp_vector(IRKOper_.Height()),
    R00(R00_), R01(R01_), R10(R10_), R11(R11_),
...
JacDiagBlockTriPrec::JacDiagBlockTriPrec(const JacDiagBlock &BlockOper_,
...
JacDiagBlockTriPrec::JacDiagBlockTriPrec(const JacDiagBlock &BlockOper_, double R10_,
...
JacDiagBlockTriPrec::JacDiagBlockTriPrec(const JacDiagBlock &BlockOper_, double R10_, Vector Y10_,
    int prec00_idx_, int prec11_idx_,
    bool identity_=false)
    : Solver(BlockOper_.Height()), BlockOper(BlockOper_),
        identity(identity_), temp_vector(BlockOper_.Offsets()[1]),
...
inline void JacDiagBlockTriPrec::Mult(const Vector &x_scalar, Vector &y_scalar) const {
...
TriJacSolver::TriJacSolver(IRKStageOper &StageOper_, int jac_update_rate_, int gamma_idx_,
    ...
    : Solver(StageOper_.Height()), StageOper(StageOper_),
    printlevel(solver_params1.printlevel),
    jac_update_rate(jac_update_rate_), gamma_idx(gamma_idx_),
    offsets(StageOper_.RowOffsets()), x_block(StageOper_.RowOffsets()),
    b_block(StageOper_.RowOffsets()), b_block_temp(StageOper_.RowOffsets()),
    x_block_temp(StageOper_.RowOffsets()), temp_vector1(StageOper_.RowOffsets()[1]),
    temp_vector2(StageOper_.RowOffsets()[1]), krylov_solver1(NULL),
    krylov_solver2(NULL), multiple_krylov(false), Z_solver(Z_solver_),
    Z_prec(Z_prec_), schur_precondition(schur_precondition_)
{
