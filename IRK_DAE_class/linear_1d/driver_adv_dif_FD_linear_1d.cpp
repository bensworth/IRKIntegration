...
/** Script to test out GMRES convergence theory for 2x2 systems. I did some theory
    that predicted GMRES convergence for SPSD and skew symmetric spatial operators 
    which apply for periodic, central FD-discretizations of diffusion and advection
    operators, respectively.

    Here there's the option to precondition the Schur complement with AMG (which 
    doesn't work ar all for the advection problem), or using an FFT solver that
    only works in serial and for 1D problems. I set up the FFT solver so that I 
    had a robust preconditioner for the advection problem since AMG falls over. While
    the FFT solver isn't necessarily practical (e.g., we can use it to exactly invert
    the Schur complement at the same expense of applying the preconditioner), I thought
    it was useful because it's robust for the advection problem, and the theory assumes
    the preconditioner is applied exactly rather than approximately as with AMG, for example.
    
    However, the problem is that after a few iterations (usually less than 5, say) GMRES
    is converging exactly, with the relative residual dropping to machine zero. So I think
    the exact application of the preconditioner means the preconditioned operator has a 
    very low degree minimal polynomial. Anyway, this exact convergence occurs before
    any asymptotic convergence can be observed, so I cannot really test out the theory
    to see if it applies to the case where we inexactly apply the preconditioner.
*/


// Sample runs:
// Specify the ratio beta/eta by the parameter -eig
// -prec 0 == AMG preconditioner, -prec 1 == FFT preconditioner
//  mpirun -np 1 ./driver_adv_dif_FD_linear_1d -jac 0 -jacu 0 -f 0 -d 1 -ex 2 -save 1 -tf 1e-5 -np 2 -nmaxit 20 -nrtol 1e-10 -natol 1e-10 -krtol 1e-12 -katol 1e-20 -kp 1 -ax 0.0 -mx 1.0 -l 8 -o 4 -dt -1 -t 14 -prec 1 -gamma 0 -eig 1
...
private:    
    ...
    map<int, int> B_hash;           // Hash table for mapping index passed with BuildPreconditioner() or ImplicitPrec() to correct entries in array above
    int B_index;                    // Index that was set with previous call to BuildPreconditioner()
...
public:
    ...
    AdvDif(Preconditioner prec_,
            IRKOperator::ExplicitGradients gradientsType, 
    ...
    inline void ImplicitPrec(const Vector &x, Vector &y) const {
        ...
        } else if (prec == Preconditioner::FFT) {
            MFEM_ASSERT(FFTSolver[B_hash.at(B_index)], 
                "AdvDif::ImplicitPrec() Must first set system! See SetSystem()");
            FFTSolver[B_hash.at(B_index)]->Mult(x.GetData(), y.GetData());
        }
    }  
    ...
    inline void ImplicitPrec(int index, const Vector &x, Vector &y) const {
        if (prec == Preconditioner::AMG) {
            ...
            MFEM_ASSERT(FFTSolver[B_hash.at(index)], 
                "AdvDif::ImplicitPrec() Must first set system! See SetSystem()");
            FFTSolver[B_hash.at(index)]->Mult(x.GetData(), y.GetData());
        }
    } 
    ...
    void BuildPreconditioner(int index, double dt, double gamma, int type);    
    ...
    /** Assemble preconditioner for gamma*M - dt*<weights,{N'}> that's applied by
        by calling: 
            1. ImplicitPrec(.,.) if no further calls to BuildPreconditioner() are made
            2. ImplicitPrec(index,.,.) */
    void BuildPreconditioner(int index, double dt, double gamma, Vector weights);
...
int main(int argc, char *argv[])
{
    ...
    args.AddOption(&dim, "-d", "--dim",
     ...
     /* Time integration */
    args.AddOption(&beta_on_eta, "-eig", "--beta_on_eta",
                  "Value of beta/eta with fudged data.");
    ...
    /// --- Solver parameters --- ///   
    args.AddOption(&prec_temp, "-prec", "--Linear preconditioner", "0==AMG, 1==FFT-based solver");
    ...
    args.AddOption(&AMG.maxiter, "-amgiter", "--AMG-max-iterations", "AMG: Maximum number of iterations");
    ...
    AdvDif SpaceDisc(static_cast<Preconditioner>(prec_temp), static_cast<IRKOperator::ExplicitGradients>(gradientsType), 
    ...
    RKData * ButcherTableau; 
    if (beta_on_eta == -1.0) {
        ButcherTableau = new RKData(static_cast<RKData::Type>(RK_ID)); 
    } else {
        ButcherTableau = new RKData(beta_on_eta); 
    }
...
AdvDif::AdvDif(Preconditioner prec_,
        IRKOperator::ExplicitGradients gradientsType, 
        FDMesh &Mesh_, int fluxID, Vector alpha_, Vector mu_, 
        int order, FDBias advection_bias) 
...
/** Assemble preconditioner for gamma*M - dt*L' that's applied by
    by calling: 
        1. ImplicitPrec(.,.) if no further calls to BuildPreconditioner() are made
        2. ImplicitPrec(index,.,.) */
void AdvDif::BuildPreconditioner(int index, double dt, double gamma, int type) {
    MFEM_ASSERT(Gradient, "AdvDif::BuildPreconditioner() Gradient not yet set!");

    std::cout << "Build prec for index = " << index << '\n';
    ...
    if (prec == Preconditioner::AMG) {
        ...
        if (!B_prec[B_hash[index]]) {
            ...
            amg_solver->SetMaxIter(AMG.maxiter); 
    ...
    // Construct FFT solver    
        ...
        if (B_hash.count(index) > 0) {
            ...
            if (B_params[B_hash[index]].dt != dt || 
                B_params[B_hash[index]].gamma != gamma ||
                !B_params[B_hash[index]].JacUpToDate) { 
                delete FFTSolver[B_hash[index]];
                FFTSolver[B_hash[index]] = NULL;
                delete B_mats[B_hash[index]];
                B_mats[B_hash[index]] = NULL;
            }    
        ...
        /*  No preconditioner previously built with this index. Create space for a 
            ...
            FFTSolver.Append(NULL);
            B_mats.Append(NULL);
            B_hash[index] = FFTSolver.Size()-1;
            B_params.Append(Prec_params()); 
        }    
        
        // Build a new preconditioner 
        if (!FFTSolver[B_hash[index]]) {
            ...
            // Get first row of B. Need to cast to HYPRE matrix to access data...
            hypre_ParCSRMatrix * BB = static_cast<hypre_ParCSRMatrix *>(*B);
            
            
            int N = B->GetGlobalNumRows();
            int start = BB->diag->i[0];
            int end = BB->diag->i[1];
            
            int nnz_stencil = end - start;
            
            double * data = new double[nnz_stencil];
            int * colinds = new int[nnz_stencil];
            
            for (int k = start; k < end; k++) {
                data[k] = BB->diag->data[k];
                colinds[k] = BB->diag->j[k];
            }
            
            // Initialize FFT solver!
            CircSolver * FFTPrec = new CircSolver(N, nnz_stencil, colinds, data, CircSolver::StencilOrientation::ROW);
            
            delete[] data;
            delete[] colinds;
            
            //delete BB;
            //std::cout << "FFT solver assembled?" << '\n';
            
            // Store pointers for solver and matrix in Arrays
            FFTSolver[B_hash[index]] = FFTPrec;
            B_mats[B_hash[index]] = B;
        }
        ...
        //     std::cout << BB->diag->data[i] << '\n';
        // }
        // for (int i = start; i < end; i++) {
        //     std::cout << BB->diag->j[i] << '\n';
        // }
...
void AdvDif::BuildPreconditioner(int index, double dt, double gamma, Vector weights) {
    MFEM_ASSERT(Gradients.Size() > 0, 
        "AdvDif::BuildPreconditioner() Gradients not yet set!");
...
AdvDif::~AdvDif()
{
    ...
    */
    ...
    for (int i = 0; i < FFTSolver.Size(); i++) delete FFTSolver[i];
}