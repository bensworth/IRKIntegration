...
\tcb{Algebra gets complicated, maybe start by setting $\gamma = \sqrt{\eta^2+\beta^2}$
or $\gamma = \eta$, where in each case one term drops and the analysis is simpler.}
...
\end{align}
%
Suppose $-\mathcal{L}$ is SPD with a spectrum $\subset (0,\infty)$.
...
such that \eqref{eq:gamma0} is also SPD ($\gamma$ such that this does not hold
...
We make the assumption that $\gamma \geq \eta $, and consider
the problem
%
...
that $\lambda_-\not\in(0,\infty)$. From \eqref{eq:roots}, this is equivalent to saying that
%
\begin{align*}
