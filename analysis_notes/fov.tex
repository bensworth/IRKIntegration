...
\end{align} 
Assuming ${\cal L}$ is diagonalizable, the spectrum ${\cal F}$ of ${\cal P}_{\gamma}$ is given by
\begin{align}
{\cal F}(\lambda; \gamma) = \frac{(\eta + \mathrm{i} \lambda)^2 + \beta^2}{(\gamma + \mathrm{i} \lambda) (\eta + \mathrm{i} \lambda)}.
\end{align}
Since ${\cal L}$ is normal, ${\cal P}_{\gamma}$ is too, and, thus, has a condition number
\begin{align}
\kappa ({\cal P}_{\gamma}) = \frac{\max \limits_{\lambda \in [-c, c]}  |{\cal F}(\lambda)|}{\min \limits_{\lambda \in [-c, c]}  |{\cal F}(\lambda)|}.
\end{align}

To analyze the extrema of $|{\cal F}|$ with respect to $\lambda$, we look at its square since it's easier,
\begin{align}
{\cal G}(\lambda) 
\coloneqq
|{\cal F}(\lambda)|^2 
=
{\cal F}^{*}(\lambda) {\cal F}(\lambda)
&=
...
\kappa^2 ({\cal P}_{\gamma}) = \frac{\max \limits_{\lambda \in [-c, c]}  {\cal G}(\lambda)}{\min \limits_{\lambda \in [-c, c]}  {\cal G}(\lambda)}.
\end{align}
...
\item For simplicity, let's just consider $c \to \infty$. While this won't arise in practice (where $c \sim \delta t / h$), it's helpful from an analysis point of view, since determining whether ${\cal G}$ for some arbitrary $c < \infty$ is an extremum will be too complicated. However, note that considering $c \to \infty$ results in an upper bound on the condition number for finite $c$, since the min/max of $|{\cal F}|$ for $\lambda \in [-c,c]$ are bounded below/above by those in $(-\infty, \infty)$. Also, I think letting $c \to \infty$ is not such a bad assumption for big time steps, like $\delta t \sim 10 \delta x$. 

Observe that as $c \to \infty$, ${\cal G}$ approaches unity: For $\beta > 0$,
\begin{align}
\label{eq:Gunity}
\lim \limits_{\lambda \to \pm \infty} {\cal G}(\lambda) 
= 
...
\end{cases}
\end{align}
Based on this result, and the implications it has for the coming analysis, we're going to assume that 
\begin{align} 
\label{eq:gam_ass}
\gamma > \sqrt{\max(0, \eta^2 - 2 \beta^2)}.
\end{align}
Furthermore, we're really only interested in the case where $\beta$ is at least as big as $\eta$, so this allows us to take $\gamma$ down to zero. As such, I don't think it's worth analyzing the other case...
Note we include the $\max$ here with 0 so that $\gamma^2  > 0$ since $\gamma \in \mathbb{R}_+$. \tcr{Come back and think about the $\gamma = \eta$ case when $\beta = 0$.}

\item ${\cal G}(\lambda)$ is symmetric about $\lambda  = 0$, thus:
\begin{enumerate}
\setlength \itemsep{1ex}

\item We need only analyze $\lambda \in [0, \infty)$ since the extrema are also symmetric

\item \underline{$\lambda = 0$ is a turning point} since ${\cal G}$ is not constant (unless $\beta = 0$ and $\gamma = \eta$) and is symmetric about $\lambda = 0$. For reference, the value of ${\cal G}(0)$ is
\begin{align}
{\cal G}(0) = \left[ \frac{1}{\gamma} \left( \eta +  \frac{\beta^2}{\eta} \right) \right]^2.
\end{align}
Notice that
\begin{align}
{\cal G}(0) 
...
{\cal O}(\lambda^4)
\end{align}
If the ${\cal O}(\lambda^2)$ term is positive, $\lambda = 0$ is a local minimum since we approach ${\cal G}(0)$ from above. Conversely, if this term is negative, $\lambda = 0$ is a local maximum since we approach ${\cal G}(0)$ from below. This can be restated as
\begin{align}
...
Noting that $\eta^4 \geq \eta^4 - 4 \eta^2 \beta^2 - \beta^4$, a sufficient condition that $\lambda = 0$ is always a local maximum is thus
\begin{align} \label{eq:suff_0max}
\gamma \leq \eta + \frac{\beta^2}{\eta} < {\cal K}.
\end{align}
Note that for $\beta = 0$, and $\gamma = \eta$, ${\cal G}(\lambda) \equiv 1$, such that $\lambda = 0$ is a global maximum. 

Since ${\cal K} > \eta + \beta^2 / \eta$, ${\cal G}(0) < 1$ when it is a local minimum.


\item Taking the  derivative of ${\cal G}$, we have
\begin{align}
\frac{\partial {\cal G}}{\partial \lambda} 
= 
-\lambda
\frac{
...
\end{align}
We immediately see the critical point $\lambda = 0$.

\indent The denominator is always positive, and, so, critical points of ${\cal G}$ occur at roots of the numerator, which we note is a quadratic function in the variable $\zeta \coloneqq \lambda^2 \geq 0$, which we write as
\begin{align}
...
\quad
b \coloneqq - \big[ 2(\beta^2 + \eta^2)^2 - 2 \eta^2 \gamma^2  \big], \\
c \coloneqq \big( \eta^4 - 4 \eta^2 \beta^2 - \beta^4 \big) \gamma^2  - \eta^2 (\eta^2 + \beta^2)^2.
\end{gathered}
\end{equation}
Thus, the four remaining (other than $\lambda = 0$) possible critical points of ${\cal G}$ occur at $\lambda = \pm \sqrt{\zeta_{\pm}}$, where $Q(\zeta_{\pm}) = 0$. Note, however, that critical points with $\zeta_{-} < 0$ or $\zeta_+ < 0$ are not valid since they'd correspond to imaginary values of $\lambda$.

Due to assumption \eqref{eq:gam_ass} resulting in ${\cal G}$ approaching 1 from below as $\lambda \to \infty$, when ${\cal G}(0)$ is a local maximum, only one of $\zeta_-$ and $\zeta_+$ can be a turning point, and it must be a local minimum; note that the other critical point is not necessarily invalid/non-existent, it could be a point of inflection. Similarly, when ${\cal G}(0)$ is a local minimum, either both $\zeta_{\pm}$ are local extrema (one being a max and the other a min), or neither of them are local extrema, in which case one or both could be points of inflection.

Let's examine the signs of the coefficients in \eqref{eq:abc_def}. Note $a > 0$ always by assumption \eqref{eq:ass1}. For the second coefficient, $b < 0$ for $\gamma < \eta + \beta^2 / \eta$, $b = 0$ for $\gamma = \eta + \beta^2 / \eta$, and $b > 0$ for $\gamma > \eta + \beta^2 / \eta$. Notice that $c$ exactly appeared in the Taylor expansion \eqref{eq:G_taylor}; this is not coincidence: $c$ is proportional to ${\cal G}_{\lambda}(0)$. Thus, there is a direct correspondence between the sign of $c$ and the nature of ${\cal G}(0)$: If ${\cal G}(0)$ is a local maximum $c < 0$, and conversely, if ${\cal G}(0)$ is a local minimum $c > 0$.

Now, the roots of $Q$ may be expressed as
\begin{align} \label{eq:zeta_pm}
\zeta_{\pm} = \frac{-b^2 \pm \sqrt{b^2 - 4ac}}{2a}, \quad a > 0.
\end{align}
Computing the discriminant of $Q$ gives
\begin{align*}
b^2 - 4ac = 4 \beta^2 (\beta^2 + 4 \eta^2) \big[ \beta^4 + (\gamma^2 - \eta^2)^2 + 2 \beta^2(\gamma^2 + \eta^2) \big] > 0,
\end{align*}
indicating that $\zeta_{\pm}$ are always real.


Now we examine the signs of these roots to understand the nature of the critical points. Recall that if critical points $\zeta_{\pm}  < 0$ then they're non-existent, so we examine when this happens. 
\begin{enumerate}
\setlength{\itemsep}{2ex}

\item From \eqref{eq:zeta_pm}, $\zeta_- < 0$ for $b > 0$, and for $c < 0$ when $b < 0$. Working this through, and noting that ${\cal G}(0)$ is never a local minimum for $\gamma < \eta + \beta^2 / \eta$ \eqref{eq:suff_0max}, we find that $\zeta_- < 0$ always, and so it is never a critical point.

\item From \eqref{eq:zeta_pm}, $\zeta_+ > 0$ for $b < 0$, and for $c < 0$ when $b > 0$. Working this through, we find $\zeta_+ > 0$ when ${\cal G}(0)$ is a local maximum, and $\zeta_+ < 0$ when ${\cal G}(0)$ is a local minimum. Thus, $\zeta_+$ is a local minimum, which is always less than unity, when ${\cal G}(0)$ is a local maximum. And $\zeta_+$ is not a critical point when ${\cal G}(0)$ is a local minimum.

\end{enumerate}



\item Combining the above discussions, we can express the extrema of ${\cal G}$ as follows. The minimum of ${\cal G}$ is
\begin{align}
\min \limits_{\lambda \in \mathbb{R}} {\cal G}(\lambda)
=
\begin{cases}
{\cal G}(0) < 1, \quad &\textrm{ if } {\cal G}(0) \textrm{ is a local minimum}, \\
{\cal G}(\sqrt{\zeta_+}) < 1, &\quad \textrm{else}.
\end{cases}
\end{align}
%
The maximum of ${\cal G}$ is
\begin{align}
\max \limits_{\lambda \in \mathbb{R}} {\cal G}(\lambda)
=
\begin{cases}
{\cal G}(0) \geq 1, \quad & 0 < \gamma \leq \eta + \frac{\beta^2}{\eta}, \\
1, \quad & \eta + \frac{\beta^2}{\eta} \leq \gamma  < \infty.
\end{cases}
\end{align}

Noting that ${\cal G}(0)$ is always a local maximum for $\gamma \leq \eta + \beta^2/\eta$, we can write condition number \eqref{eq:cond_skew_def} as
\begin{align} \label{eq:cond_}
\kappa^2({\cal P}_{\gamma}) =
\begin{cases}
\displaystyle
\frac{{\cal G}(0)}{{\cal G}(\sqrt{\zeta_+})}, 
\quad \sqrt{\max(\eta^2 - 2 \beta^2, 0)} < \gamma \leq \eta + \frac{\beta^2}{\eta}, \\[2.5ex]
\begin{cases}
\displaystyle
\frac{1}{{\cal G}(\sqrt{\zeta_+})}, \quad &\textrm{ if } {\cal G}(0) \textrm{ is a local minimum}, \\[2.5ex]
\displaystyle
\frac{1}{{\cal G}(0)}, \quad &\textrm{ if } {\cal G}(0) \textrm{ is a local minimum}, \quad \\
\end{cases},
\quad \eta + \frac{\beta^2}{\eta} \leq \gamma < \infty
\end{cases}
\end{align}
...
\begin{theorem}
Condition number \eqref{eq:cond_} is minimized with respect to $\gamma$ for 
\begin{align} 
...
\tcr{TODO: write out proof for this next bit...} First we establish that for $\gamma > \eta + \frac{\beta^2}{\eta}$ condition number \eqref{eq:cond_} is strictly increasing, and thus cannot be minimized in this interval. 

Having established that $\kappa(\gamma)$ is increasing for $\gamma > \eta + \frac{\beta^2}{eta}$, we now show that it is strictly decreasing for $\gamma \in (\sqrt{\max(\eta^2 - 2 \beta^2, 0)}, \eta + \beta^2\eta]$. Again, we work with the square of condition number \eqref{eq:cond_}, which in this interval, can be expressed as
\begin{align}
\label{eq:cond_2}
\kappa^2(\gamma) = 
...
For $\beta > 0$, the derivative is given by and can be bounded above as follows
\begin{align*}
[\kappa^2(\gamma)]' 
&= 
...
\end{align*}
Applying both of the above results, we find $[\kappa^2]' < 0$ for $\gamma \leq \eta + \beta^2 / \eta$. Note that $[\kappa^2(\gamma)]' = 2 \kappa \kappa'$, and since $\kappa > 0$, $[\kappa^2]' < 0$ implies $\kappa' < 0$.
%
Since $\kappa$ is strictly decreasing for $\gamma \leq \eta + \beta^2/\eta$, and $\kappa$ is increasing for $\gamma > \eta + \beta^2/\eta$, it is minimized at the interface, $\gamma = \gamma_* = \eta + \beta^2/\eta$, as in \eqref{eq:gamma_opt_skew}. Evaluating \eqref{eq:cond_2} at $\gamma_*$ gives result \eqref{eq:cond_opt_skew}.


\end{proof}


Another case of interest is $\gamma = \eta$. Evaluating this condition number we find
\begin{align}
\kappa({\cal P}_{\eta}) = \frac{1}{2} \sqrt{4 + \left( \frac{\beta}{\eta} \right)^2} \left[ 1 + \left( \frac{\beta}{\eta} \right)^2 \right].
\end{align}
Clearly $\kappa({\cal P}_{\eta + \beta^2/\eta}) < \kappa({\cal P}_{\eta})$ by a long way! However, $\kappa({\cal P}_{\eta + \beta^2/\eta})$ is only just smaller than the bound on it that Ben derived using the general analysis! I guess this says that 1: That bound is fairly tight. 2: Skew symmetric matrices lead to a fairly poorly conditioned ${\cal P}_{\gamma}$, in general, since we saw for SSPD matrices that the operator has a much smaller condition number.


\item \tcr{TODO: write out proof for this}. For $\gamma \geq 0$, $g$ has 2 critical points: $\gamma = 0$, and $\gamma = \sqrt{2 \beta^2 - \eta^2}$. Note that the second of the two does not exist if $2 \beta^2 - \eta^2 < 0$. Anyway, we only consider $\gamma > \max (0, \sqrt{2 \beta^2 - \eta})$, as per assumption \eqref{eq:ass1}. Thus, in the interval we care about, $g$ is monotone. Moreover, we can show it's decreasing.
...
\tcr{Regarding taking $c \to \infty$. Can I quantify the difference between $c$ and $\zeta_+$, which is the min? E.g., $\zeta_+ = {\cal O}(...)$, so  that if  $c$ is of this order, or bigger, then the assumption of taking $c \to \infty$ is not so bad because it's not min anyways.
%
If $c < \zeta_+$, then it is the minimum over the domain... Can formulate $\kappa$ for this case easily, since $c$ is always the minimum? Well provided 0 doesn't turn into the minimum, that is. But $\zeta_+$ depends on $\gamma$... So not sure how this works...
Alternatively, can assume $c$ corresponds to the minimum, and work out what the optimum would be for gamma, then just directly compare the two condition numbers...
}

\tcr{Note the bound here is big in beta/eta, as is much larger than in the linear algorithm for general operators. So, why is this the case?}
