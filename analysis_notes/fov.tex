...
\end{align*}
To analyze the extrema of $|{\cal F}|$ we work with its square since it's easier,
\begin{align} 
\label{eq:G_def}
...
\item From \eqref{eq:zeta_pm}, $\zeta_- < 0$ for any $b > 0$, and for $b<0$, we'd require $c < 0$ to have $\zeta_- < 0$. Working this through, and noting that ${\cal G}(0)$ is never a local minimum for $\gamma < \eta + \beta^2 / \eta$, as in \eqref{eq:G0_class} and \eqref{eq:suff_0max}, we find that $\zeta_- < 0$ always, and so it is never a critical point.

\item From \eqref{eq:zeta_pm}, $\zeta_+ > 0$ for any $b < 0$, and for $b > 0$, we'd require $c < 0$ to have $\zeta_+ > 0$. Working this through, we find $\zeta_+ > 0$ when ${\cal G}(0)$ is a local maximum, and $\zeta_+ < 0$ when ${\cal G}(0)$ is a local minimum. Thus, $\zeta_+$ is a local minimum when ${\cal G}(0)$ is a local maximum. And $\zeta_+$ is not a critical point when ${\cal G}(0)$ is a local minimum. Moreover, we know that when $\lambda = \sqrt{\zeta_+}$ is a local minimum of ${\cal  G}$, ${\cal G}(\sqrt{\zeta_+}) < 1$ due to ${\cal G}$ approaching unity from below as $\lambda \to \infty$, as in \eqref{eq:ass1}, and ${\cal G}(0)$ being the only other turning point.
...
\underline{Case 2: $\sqrt{\max(\eta^2 - 2 \beta^2, 0)} < \gamma \leq \eta + \beta^2/\eta$} \quad Having now established that $\kappa^2(\gamma)$ is strictly increasing for $\gamma > \eta + \beta^2/\eta$, we now show that it is strictly decreasing for $\gamma \in (\sqrt{\max(\eta^2 - 2 \beta^2, 0)}, \eta + \beta^2/\eta)$. Again, we work with the square of condition number \eqref{eq:cond_val_skew}, which in this interval can be expressed as
\begin{align}
\label{eq:cond_2}
\kappa^2(\gamma) = 
...
\end{align}
Clearly $\kappa({\cal P}_{\eta + \beta^2/\eta}) < \kappa({\cal P}_{\eta})$. However, $\kappa({\cal P}_{\eta + \beta^2/\eta})$ is only just smaller than the bound on it that Ben derived using the general analysis, which was 
\begin{align}
\kappa(\mathcal{P}_{\gamma_*}) \leq 2+\frac{\beta^2}{2\eta^2} - \frac{\beta^2}{2\eta^2+2\beta^2}.
\end{align}
And note that actually, as $\beta/\eta \to \infty$, this bound is equal to \eqref{eq:cond_opt_skew}.
...
See Figure \ref{fig:conds_all} for a comparison of the condition numbers for the SS and SSPD cases.

\begin{figure}[!htb]
\label{fig:conds_all}
\centering
\includegraphics[width=0.65\textwidth]{conds_all}
\caption{Condition numbers for SSPD and SS matrices ${\cal L}$ as a function of $(\beta/\eta)^2$.}
\end{figure}
...
\\
\mu_{\max} \left( {\cal P}_{\eta + \beta^2/\eta} {\cal P}_{\eta + \beta^2/\eta}^\top \right) 
&= {\cal G}_{\eta + \beta^2/\eta}(0) 
= 
1,
\end{aligned}
\end{equation}
...
\\
{\cal H}_{\eta + \beta^2/\eta}(\lambda) 
&= 
\eta^2 \frac{\beta^4 + 2 \eta^2 \beta^2 +  (\eta^2 + \lambda^2)^2}{(\eta^2 + \lambda^2) \left[ (\eta^2 + \beta^2)^2 + (\eta \lambda)^2 \right]}.
\end{align*}
Notice that ${\cal H}_{\eta + \beta^2/\eta}(\lambda) > 0$, meaning that the symmetric part of ${\cal P}_{\gamma}$ is always positive definite.
...
\quad
\underset{\lambda \in \mathbb{R}}{\min} \,  {\cal H}_{\eta + \beta^2/\eta}(\lambda) =  \frac{2}{\left( \beta/\eta \right)^2 + 2}. 
\end{align}
...
{\cal Z}_{\eta + \beta^2 / \eta} &= \frac{ \left( \frac{\beta}{\eta} \right)^2}{\left( \frac{\beta}{\eta} \right)^2 + 2}.
\end{align}

\begin{figure}[!htb]
\label{fig:gmres_bounds}
\centering
\includegraphics[width=0.65\textwidth]{gmres_bounds}
\caption{GMRES bound ${\cal Z}_{\gamma}$ from \eqref{eq:gmres_eig_bound} for SSPD and SS matrices ${\cal L}$ as a function of $(\beta/\eta)^2$.}
\end{figure}

See Figure \ref{fig:gmres_bounds} for a plot of the GMRES bounds which also includes the SSPD convergence bounds. Note that in the case of SPD ${\cal P}_{\gamma}$, ${\cal Z}_{\gamma} = (\kappa^2 - 1)/\kappa^2$.
