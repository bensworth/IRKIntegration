...
\end{align*}
which we have been \textit{choosing} to precondition with the operator $(\gamma I - {\cal L})^{-2}$, $\gamma \in \mathbb{R}_+$. But, we could also consider a more general preconditioner of the form $(\delta I - {\cal L})^{-1} (\gamma I - {\cal L})^{-1}$, for $\delta, \gamma \in \mathbb{R}_+$. In which case, the preconditioned operator takes the form
\begin{align} \label{eq:P_gen_lin}
{\cal P}_{\delta, \gamma}^{1\times1} \coloneqq \big[ (\eta I  - {\cal L})^2 + \beta^2 I \big] (\delta I - {\cal L})^{-1} (\gamma I - {\cal L})^{-1}, \quad \delta, \gamma \in \mathbb{R}_+.
\end{align}
...
\end{align*}
which we've been choosing to precondition with the operator $(\gamma I - {\cal L})^{-1}$, $\gamma \in \mathbb{R}_+$. So, in this case, the preconditioned operator is given by
\begin{align}
\nonumber
...
{\cal P}_{\delta, \gamma} \coloneqq [(\eta I  - {\cal L})^2 + \beta^2 I] (\delta I - {\cal L})^{-1} (\gamma I - {\cal L})^{-1}, \quad \delta, \gamma \in \mathbb{R}_+,
\end{align}
in which $[(\eta I  - {\cal L})^2 + \beta^2 I]$ is preconditoned with $(\delta I - {\cal L})^{-1} (\gamma I - {\cal L})^{-1}$.
%
...
\kappa(\mathcal{P}_{\delta, \gamma_*}) \leq \frac{\delta + \gamma_*}{2 \eta}.
\end{align}
...
Substituting $\gamma_*$ \eqref{eq:gamma*_gen} into condition number bound \eqref{eq:kappa_gen_gamma*_bound} yields
\begin{align*}
\kappa({\cal P}_{\delta, \gamma_*}) \leq \frac{1}{2 \eta} \left(\delta + \frac{\eta^2 + \beta^2}{\delta}\right).
\end{align*}
Differentiating this expression, for $\delta > 0$ we see that it has only one critical point at $\delta = \sqrt{\eta^2+ \beta^2}$. Since this function is increasing as $\delta \to 0^+$ and $\delta \to \infty$, this critical point must be a local minimum. Therefore, the tight upper bound \eqref{eq:kappa_gen_gamma*_bound} is minimized when $\delta = \sqrt{\eta^2+ \beta^2}$. Substituting $\delta =  \sqrt{\eta^2+ \beta^2}$ into \eqref{eq:gamma*_gen} yields $\gamma_* = \sqrt{\eta^2 + \beta^2}$, and substituting $\delta = \gamma_* = \sqrt{\eta^2 + \beta^2}$ into \eqref{eq:kappa_gen_gamma*_bound} yields bound \eqref{eq:kappa_gammastar,gammastar}.
\end{proof}

\begin{corollary}[Optimal preconditioning in the $2 \times 2$ setting]
With respect to a tight upper bound on its condition number, the optimal value of $\gamma$ in the preconditoned operator \eqref{eq:P_gen_nonlin} is $\gamma = \gamma_* =  \eta + \beta^2 / \eta$, and the condition number of the corresponding preconditioned operator is tightly bounded by
\begin{align}
...
\quad
\Vert {\cal P}_{\delta,\gamma_*}^{-1} \Vert \leq \frac{\delta + \gamma_*}{2 \eta}
\end{align}
...
We now show that bound \eqref{eq:kappa_gen_gamma*_bound} on $\kappa^2({\cal P}_{\delta,\gamma_*})$
