...
\end{align*}
%
with two particular cases of interest
%
\begin{align*}
\|\mathcal{P}_\eta\| & \leq 1 + \frac{\beta^2}{\eta^2}, \\
\|\mathcal{P}_{\gamma_*}\| & \leq 3 - 2\frac{\eta}{\sqrt{\eta^2+\beta^2}}.
\end{align*}
%
...
& = \min_{\mathbf{v}\neq\mathbf{0}}
	...
	{\|(I - \tfrac{1}{\gamma}\mathcal{L})^{2}\mathbf{w}\|^2}.\label{eq:min0}
\end{align}
%
Deriving bounds on the above operator for general $\gamma$ is quite difficult.
However, we will consider two representative cases.
...
$\gamma_*$ provides a unique simplification here as well. From \eqref{eq:min0}, we have
%
\begin{align*}
...
% ---------------------------------------------------------------------------------------------- %
\subsubsection{General $\gamma$}

Here we consider conditioning using general $\gamma$. For ease of notation, define
%
\begin{align*}
...
\end{align*}
%
Now, collect terms in \eqref{eq:min0} and expand the norm to yield
%
\begin{align*}
...
\end{align*}
%
Note that by assumption on $\mathcal{L}\leq 0$, all terms in the numerator an
denominator are non-negative. For numerator $\mathcal{N}$ and denominator
$\mathcal{D}$, if $\widehat{K}\mathcal{N} \geq \mathcal{D}$, then
$\mathcal{N}/\mathcal{D} \geq 1 / \widehat{K}$. For 
...
$\gamma \in[\eta,\tfrac{\eta^2+\beta^2}{\eta}]$, the maximum of the
above ratios ends up being $\widehat{K} := \tfrac{\gamma^2}{\eta^2}$.

The difficult ratio ends
up being $\|(1+c_I)\mathbf{w}+\tfrac{1}{\gamma^2}\mathcal{L}^2\mathbf{w}\|^2$ and the
...
angle $\theta$. Taking the ratio of these individual terms then yields
%
...
cancel a $1/\|\mathbf{w}\|^2$ from the numerator and denominator, and
...
\end{align}
%
Clearly for $c_I\geq 0$ (corresponding to $\gamma \leq\sqrt{\eta^2+\beta^2}$)
and $\cos(\theta)\geq 0$, \eqref{eq:x_simp} is $\geq 1$ for all $x$. It is
...
%%%%%
\textbf{Positive $\mathcal{L}^2$}: Suppose $\mathcal{L}^2 \geq 0$ in a field
of values sense and $\gamma \leq \sqrt{\eta^2+\beta^2}$. Then the ratio in
\eqref{eq:problem} is always $\geq 1$ (that is, $\widehat{K}=1$ is sufficient
to bound $\widehat{K}\mathcal{N} \geq \mathcal{D}$) and the maximum required
$\widehat{K}$ is given by $\widehat{K} = \tfrac{\gamma^2}{\eta^2}$. Altogether,
this yields
%
\begin{align}\label{eq:gen_bound}
...
%%%%%
$\boldsymbol{\gamma_* = \sqrt{\eta^2+\beta^2}}$: Under this specific choice, we
get the nice cancellation that $c_I = 0$. Then, \eqref{eq:x_simp} equals one,
and we can again refer to the $\widehat{K}$ derived previously, and the bound
in \eqref{eq:gen_bound}
%
\begin{align}\label{eq:gammas_bound}
\|P_{\gamma}^{-1}\| & = \frac{\sqrt{\eta^2+\beta^2}}{\eta} =
	\sqrt{1+\frac{\beta^2}{\eta^2}}.
\end{align}
%
Fortunately, this is consistent with the previous section where we specifically
focus on $\gamma_* = \sqrt{\eta^2+\beta^2}$ using a slightly modified derivation.
...
For $\mathcal{L}^2 \geq 0$ or $\gamma = \gamma_* = \sqrt{\eta^2+\beta^2}$
(regardless of properties of $\mathcal{L}^2$), we have
%
\begin{align*}
...
\end{align*}
%
For the specific choice $\gamma = \gamma_*$, this simplifies to
%
\begin{align*}
