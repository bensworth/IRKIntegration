...
finite elements in time as well.\todo{more on DG in time}
...
% ------------------------------------------------------------------------------------- %
% ------------------------------------------------------------------------------------- %
\subsection{Finite-difference advection-diffusion}



% ------------------------------------------------------------------------------------- %
% ------------------------------------------------------------------------------------- %
\subsection{Discontinuous Galerkin advection-diffusion}
...
\ldots, (Q_0^\top)_{k, s} (Q_0)_{s, \ell} \Big) \in \mathbb{R}^s$ is a row vector,
$\bm{{\cal L}}' = ({\cal L}'_1, \ldots, {\cal L}'_s)$ is a column vector of the
linearized operators, and
%
\begin{align*}
\bm{c}^\top_{k,\ell} \bm{{\cal L}}' = \sum \limits_{i = 1}^s (\bm{c}_{k, \ell})_i {\cal L}'_i.
\end{align*}
%
...
a_1\mathcal{L}_1 + a_2\mathcal{L}_2 + a_3\mathcal{L}_3$:
%
\begin{align*}
...
one and off-diagonal sum to zero, wherein if $\mathcal{L}_i = \mathcal{L}_j$
...
simply a measure of commutation, $0.121(\mathcal{L}_1 - \mathcal{L}_2)$.
...
memory usage), or simply ignored under the assumption that $\mathcal{L}_1 -
\mathcal{L}_2$ is ``small'' in some sense. Even for reasonably stiff 
problems, the operator often does not change significantly between
...
holds for other methods as well. For example, the constants for four-stage
Gauss are given by:
%
\begin{align*}
...
\end{align*}
%
Analogous to the twi-stage case, note that the diagonal blocks are largely
defined by $\mathcal{L}$ evaluated at a single time point (coefficient
here shown in bold). Moreover, not only are the off-diagonal constants
...
\end{align}
%
via block backward substitution. In addition to the simplified Newton
...
block of $\widetilde{P}$ contains only one matrix from $\bm{{\cal L}}'$.
