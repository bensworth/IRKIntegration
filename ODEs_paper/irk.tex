...
\tcp{
MATLAB computes the Schur decomposition of $2 \times 2$ matrix as
\[
A = \tilde{Q} \tilde{R} \tilde{Q}^\top
=
\tilde{Q}
\begin{bmatrix}
\eta & \phi \\
...
Q^\top \quad (**).
\]
Now, $\tilde{R}$ and $R$ are of course similar, and, in fact, we have
\[
\tilde{R} = P^{-1} R P, \quad 
P = 
\begin{bmatrix}
...
\[
A = \tilde{Q} (P^{-1} R P) \tilde{Q}^\top 
= (\tilde{Q}P^{-1}) R (\tilde{Q} P^{-1})^{-1}
\equiv W R W^{-1}. \quad (****)
\]
However, $(****)$ is not a Schur decomposition because $P$ is not orthogonal ($P^\top = P \neq P^{-1}$), and so neither is $W = \tilde{Q} P^{-1}$. This says that given the  Schur decomposition $(*)$ that we compute in MATLAB, we cannot transform it into  Schur decomposition $(**)$ that we actually want because $\tilde{R}$ and $R$ are not \textbf{unitarily similar}.}

Furthermore, this says that we cannot do a Schur decomposition of an arbitrary $2 \times 2$ matrix $A$ in the form of $(**)$? Here's a proof by counter example: Take $A = \tilde{R}$, clearly we cannot write $\tilde{R} = Q R Q^\top$ because $\tilde{R}$ and $R$ are not unitarily similar (see non-unitary $P$ in  $(***)$).

So, I think the best we can do for a matrix $A$ is the real, non-unitary similarity transform given by $(****)$; however, I think this is fine because the algebra you did doesn't depend on $Q$ being orthogonal, apart from maybe some of the comments you made about the fully time-dependent case regarding the sums of the coefficients being unity?

\tcp{
I also note that the Schur complement of $\tilde{R} \otimes I - I \otimes \widehat{\cal L}$ is still $\eta I - \widehat{\mathcal{L}} + \beta^2 (\eta I - \widehat{\mathcal{L}})^{-1}$, so does this even matter?
}

%
\begin{remark}[Symmetric operators and three-term recursion]
...
In general, we have an algorithm that is similar in essence to that of \Cref{sec:solve},
but is able to store all stage vectors and is compatible with a simplified Newton
iteration. For linear problems, the algorithm from \Cref{sec:solve} is still
...
$W(\mathcal{P}_\mu)$ is bounded by $\Omega$ as shown in \Cref{fig:bound}.
\end{corollary}
\begin{proof}
The proof follows analogous to that of \Cref{th:fov}.
\end{proof}
%
...
\begin{itemize}
	...
	off-diagonal blocks are zero, \Cref{cor:fov} extends well to a $2\times 2$
