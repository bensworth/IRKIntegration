...
approximation, which can then be easily inverted using a block backward
solve.
...
a_1\mathcal{L}_1 + a_2\mathcal{L}_2 + a_3\mathcal{L}_3$. Note in the matrices
below, the constants in diagonal brackets always sum to one and off-diagonal
sum to zero, wherein if $\mathcal{L}_i = \mathcal{L}_j$ for all $i,j$, each
of the following operators is the identity.
%
\begin{align*}
