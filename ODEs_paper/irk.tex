...
\newcommand{\TheTitle}{Fast parallel solution of fully implict\\Runge-Kutta in numerical PDEs}
...
necessary to obtain 4th-order accuracy compared with standard 4th-order SDIRK
schemes.
...
wherein we develop a method to solve
...
develop an effective preconditioning fir linear problems in \Cref{sec:solve:prec}.
...
Throughout the paper, we use the reformulation
introduced in \cite{pazner17}, where we can pull an $A_0\otimes I$ out of the
...
interested in implicit schemes and, because the majority of implciit Runge-Kutta schemes
...
field-of-values of $\mathcal{P}_\eta$, denoted $W(\mathcal{P}_\eta)$ \eqref{cor:fov},
as a measure of the preconditioning, and \Cref{cor:gmres,cor:cg} extend this 
reconditioning to prove fast convergence of GMRES and conjugate gradient (CG).
...
Assume \Cref{ass:eig} and \Cref{ass:fov} hold, that is, $\eta > 0$
and $W(\mathcal{L}) \leq 0$ \eqref{cor:fov}. Let $\mathcal{P}_\eta$ denote the
...
less slower convergence than \Cref{th:fov}.}
\end{proof}
%
...
in \Cref{tab:beta} apply here as well.\footnote{Note, for symmetric spatial
operators, the nonsymmetric matrix in \eqref{eq:block}
...
block-triangular preconditioning of the original system \todo{cite}.}
...
constraints). A field-of-values analysis is derived to guarantees rapid Krylov
